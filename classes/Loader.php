...
class Loader
{
	...
	{
		$return = ($hook = ForumFunction::get_hook('ld_fn_add_js_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
		...
		$default_options['data'] = ForumFunction::forum_trim($data);
		...
		($hook = ForumFunction::get_hook('ld_fn_add_js_pre_merge')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('ld_fn_add_js_end')) ? eval($hook) : null;
	...
	public function render_js()
	{
		...
		$return = ($hook = ForumFunction::get_hook('ld_fn_render_js_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
	...
	{
		$return = ($hook = ForumFunction::get_hook('ld_fn_add_css_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
		...
		($hook = ForumFunction::get_hook('ld_fn_add_css_pre_merge')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('ld_fn_add_css_end')) ? eval($hook) : null;
	...
	public function render_css()
	{
		...
		$return = ($hook = ForumFunction::get_hook('ld_fn_render_css_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
	...
	private function render_css_simple()
	{
		...
		$return = ($hook = ForumFunction::get_hook('ld_fn_render_css_simple_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
		...
		foreach ($libs as $key => $lib)
		{
			if ($lib['type'] == 'inline')
			{
				if ($lib['noscript'] === true)
					$output .= ForumFunction::forum_trim($this->check_conditional_comments($lib, '<noscript><style>'.$lib['data'].'</style></noscript>'))."\n";
				else
					$output .= ForumFunction::forum_trim($this->check_conditional_comments($lib, '<style>'.$lib['data'].'</style>'))."\n";
				unset($libs[$key]);
				continue;
			}
			...
			{
				if ($lib['noscript'] === true)
					$output .= ForumFunction::forum_trim($this->check_conditional_comments($lib, '<noscript><link rel="stylesheet" type="text/css" media="'.$lib['media'].'" href="'.$lib['data'].'" /></noscript>'))."\n";
				else
					$output .= ForumFunction::forum_trim($this->check_conditional_comments($lib, '<link rel="stylesheet" type="text/css" media="'.$lib['media'].'" href="'.$lib['data'].'" />'))."\n";
				unset($libs[$key]);
				continue;
			}
		}

		($hook = ForumFunction::get_hook('ld_fn_render_css_simple_end')) ? eval($hook) : null;
	...
	private function render_js_simple()
	{
		...
		$return = ($hook = ForumFunction::get_hook('ld_fn_render_js_simple_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
		...
		($hook = ForumFunction::get_hook('ld_fn_render_js_simple_end')) ? eval($hook) : null;
	...
	private function render_js_labjs()
	{
		...
		$return = ($hook = ForumFunction::get_hook('ld_fn_render_js_labjs_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
		...
		($hook = ForumFunction::get_hook('ld_fn_render_js_labjs_end')) ? eval($hook) : null;
	...
	{
		$return = ($hook = ForumFunction::get_hook('ld_fn_sort_libs_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
	...
	{
		$return = ($hook = ForumFunction::get_hook('ld_fn_check_conditional_comments_start')) ? eval($hook) : null;
		if ($return !== null)
			return $return;
