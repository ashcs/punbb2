...
class DBLayer
{
	...
	function __construct($db_host, $db_username, $db_password, $db_name, $db_prefix, $p_connect)
	{
		...
		if (!file_exists($db_name))
		{
			...
			@chmod($db_name, 0666);
			if (!file_exists($db_name))
			 ForumFunction::error('Unable to create new database \''.$db_name.'\'. Permission denied.', __FILE__, __LINE__);
		}

		if (!is_readable($db_name))
		 ForumFunction::error('Unable to open database \''.$db_name.'\' for reading. Permission denied.', __FILE__, __LINE__);

		if (!is_writable($db_name))
		 ForumFunction::error('Unable to open database \''.$db_name.'\' for writing. Permission denied.', __FILE__, __LINE__);
		...
		if (!$this->link_id)
		 ForumFunction::error('Unable to open database \''.$db_name.'\'. SQLite reported: '.$sqlite_error, __FILE__, __LINE__);
		else
			return $this->link_id;
	}
	...
	function query($sql, $unbuffered = false)
	{
		...
		if (defined('FORUM_SHOW_QUERIES') || defined('FORUM_DEBUG'))
			$q_start = ForumFunction::forum_microtime();
		...
		{
			if (defined('FORUM_SHOW_QUERIES') || defined('FORUM_DEBUG'))
				$this->saved_queries[] = array($sql, sprintf('%.5f', ForumFunction::forum_microtime() - $q_start));
	...
	function create_table($table_name, $schema, $no_prefix = false)
	{
		...
		$this->query($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	function drop_table($table_name, $no_prefix = false)
	{
		...
		$this->query('DROP TABLE '.($no_prefix ? '' : $this->prefix).$table_name) or ForumFunction::error(__FILE__, __LINE__);
	}
	...
	{
		// Grab table info
		$result = $this->query('SELECT sql FROM sqlite_master WHERE tbl_name = \''.($no_prefix ? '' : $this->prefix).$this->escape($table_name).'\' ORDER BY type DESC') or ForumFunction::error(__FILE__, __LINE__);
		...
		{
			$table_line = ForumFunction::forum_trim($table_line);
			if (substr($table_line, 0, 12) == 'CREATE TABLE')
				...
				$table['columns'][substr($table_line, 0, strpos($table_line, ' '))] = ForumFunction::forum_trim(substr($table_line, strpos($table_line, ' ')));
		}
	...
	function add_field($table_name, $field_name, $field_type, $allow_null, $default_value = null, $after_field = 0, $no_prefix = false)
	{
		...
		$this->query($tmptable) or ForumFunction::error(__FILE__, __LINE__);
		$this->query('INSERT INTO '.($no_prefix ? '' : $this->prefix).$this->escape($table_name).'_t'.$now.' SELECT * FROM '.($no_prefix ? '' : $this->prefix).$this->escape($table_name)) or ForumFunction::error(__FILE__, __LINE__);
		...
		ForumFunction::array_insert($table['columns'], $after_field, $query.',', $field_name);
		...
		// Create new table
		$this->query($new_table) or ForumFunction::error(__FILE__, __LINE__);

		// Recreate indexes
		if (!empty($table['indices']))
		{
			foreach ($table['indices'] as $cur_index)
				$this->query($cur_index) or ForumFunction::error(__FILE__, __LINE__);
		}

		//Copy content back
		$this->query('INSERT INTO '.($no_prefix ? '' : $this->prefix).$this->escape($table_name).' ('.implode(', ', $old_columns).') SELECT * FROM '.($no_prefix ? '' : $this->prefix).$this->escape($table_name).'_t'.$now) or ForumFunction::error(__FILE__, __LINE__);
	...
	function drop_field($table_name, $field_name, $no_prefix = false)
	{
		...
		//Copy content back
		$this->query('INSERT INTO '.($no_prefix ? '' : $this->prefix).$this->escape($table_name).' SELECT '.implode(', ', $new_columns).' FROM '.($no_prefix ? '' : $this->prefix).$this->escape($table_name).'_t'.$now) or ForumFunction::error(__FILE__, __LINE__);
	...
	function add_index($table_name, $index_name, $index_fields, $unique = false, $no_prefix = false)
	{
		...
		$this->query('CREATE '.($unique ? 'UNIQUE ' : '').'INDEX '.($no_prefix ? '' : $this->prefix).$table_name.'_'.$index_name.' ON '.($no_prefix ? '' : $this->prefix).$table_name.'('.implode(',', $index_fields).')') or ForumFunction::error(__FILE__, __LINE__);
	}
	...
	function drop_index($table_name, $index_name, $no_prefix = false)
	{
		...
		$this->query('DROP INDEX '.($no_prefix ? '' : $this->prefix).$table_name.'_'.$index_name) or ForumFunction::error(__FILE__, __LINE__);
	}
}
