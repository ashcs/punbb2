...
class SearchFunctions
{
    ...
    function create_search_cache($keywords, $author, $search_in = false, $forum = array(-1), $show_as = 'topics', $sort_by = null, $sort_dir = 'DESC')
    {
        ...
        $return = ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_start')) ? eval($hook) : null;
        if ($return !== null)
            return;
        
        if (utf8_strlen(str_replace(array(
            '*',
            '%'
        ), '', $author)) < 2)
            $author = '';
        ...
        if (utf8_strlen(str_replace(array(
            '*',
            '%'
        ), '', $keywords)) < FORUM_SEARCH_MIN_WORD)
            $keywords = '';
        ...
        if ($forum_user['last_search'] && (time() - $forum_user['last_search']) < $forum_user['g_search_flood'] && (time() - $forum_user['last_search']) >= 0)
            ForumFunction::message(sprintf($lang_search['Search flood'], $forum_user['g_search_flood']));
        
        if ($forum_user['is_guest']) {
            $query = array(
                'UPDATE' => 'online',
                'SET' => 'last_search=' . time(),
                'WHERE' => 'ident=\'' . $forum_db->escape(ForumFunction::get_remote_address()) . '\''
            );
        } else {
            $query = array(
                'UPDATE' => 'users',
        ...
        ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_update_last_search_time')) ? eval($hook) : null;
        ...
        if ($keywords) {
            ...
            $keywords_array = array_filter($keywords_array, '\\Punbb\\ForumFunction::validate_search_word');
                    ...
                    default:
                        {
                            ...
                            ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_get_keyword_hits')) ? eval($hook) : null;
        ...
        if ($author && $author != 'guest' && $author != utf8_strtolower($lang_common['Guest'])) {
            ...
            ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_get_author')) ? eval($hook) : null;
            $result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
            ...
            if (! empty($user_ids)) {
                ...
                ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_get_author_hits')) ? eval($hook) : null;
                $result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
        ...
        ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_get_hits')) ? eval($hook) : null;
        $result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
        ...
        ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_get_online_idents')) ? eval($hook) : null;
        $result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
        ...
        if (! empty($online_idents)) {
            ...
            ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_delete_old_cached_searches')) ? eval($hook) : null;
            $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
        }
        ...
        $ident = ($forum_user['is_guest']) ? ForumFunction::get_remote_address() : $forum_user['username'];
        ...
        ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_qr_cache_search')) ? eval($hook) : null;
        $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
        
        $return = ($hook = ForumFunction::get_hook('sf_fn_create_search_cache_end')) ? eval($hook) : null;
        if ($return !== null)
            return;
        ...
        header('Location: ' . str_replace('&amp;', '&', ForumFunction::forum_link($forum_url['search_results'], $search_id)));
        exit();
    }
    ...
    function generate_cached_search_query($search_id, &$show_as)
    {
        ...
        $return = ($hook = ForumFunction::get_hook('sf_fn_generate_cached_search_query_start')) ? eval($hook) : null;
        if ($return !== null)
            return $return;
        ...
        ($hook = ForumFunction::get_hook('sf_fn_generate_cached_search_query_qr_get_cached_search_data')) ? eval($hook) : null;
        $result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
        ...
        switch ($sort_by) {
            ...
            default:
                ...
                ($hook = ForumFunction::get_hook('sf_fn_generate_cached_search_query_qr_cached_sort_by')) ? eval($hook) : null;
                break;
        }
        ...
        if ($show_as == 'posts') {
            ...
            ($hook = ForumFunction::get_hook('sf_fn_generate_cached_search_query_qr_get_cached_hits_as_posts')) ? eval($hook) : null;
        } else {
            $query = array(
            ...
            if (! $forum_user['is_guest'] && $forum_config['o_show_dot'] == '1') {
                ...
                ($hook = ForumFunction::get_hook('sf_fn_generate_cached_search_query_qr_get_has_posted')) ? eval($hook) : null;
            }
            
            ($hook = ForumFunction::get_hook('sf_fn_generate_cached_search_query_qr_get_cached_hits_as_topics')) ? eval($hook) : null;
        }
        
        ($hook = ForumFunction::get_hook('sf_fn_generate_cached_search_query_end')) ? eval($hook) : null;
    ...
    function generate_action_search_query($action, $value, &$search_id, &$url_type, &$show_as)
    {
        ...
        $return = ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_start')) ? eval($hook) : null;
        if ($return !== null)
            return $return;
        ...
        switch ($action) {
            case 'show_new':
                ...
                if (! $forum_user['is_guest'] && $forum_config['o_show_dot'] == '1') {
                    ...
                    ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_new_topics_has_posted')) ? eval($hook) : null;
                }
                
                $url_type = $forum_url['search_new_results'];
                $search_id = $value;
                
                ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_new')) ? eval($hook) : null;
            ...
            case 'show_recent':
                ...
                if (! $forum_user['is_guest'] && $forum_config['o_show_dot'] == '1') {
                    ...
                    ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_recent_topics_has_posted')) ? eval($hook) : null;
                }
                
                $url_type = $forum_url['search_recent_results'];
                $search_id = $value;
                
                ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_recent')) ? eval($hook) : null;
            ...
            case 'show_user_posts':
                ...
                ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_user_posts')) ? eval($hook) : null;
            ...
            case 'show_user_topics':
                ...
                if (! $forum_user['is_guest'] && $forum_config['o_show_dot'] == '1') {
                    ...
                    ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_user_topics_has_posted')) ? eval($hook) : null;
                }
                
                $url_type = $forum_url['search_user_topics'];
                $search_id = $value;
                
                ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_user_topics')) ? eval($hook) : null;
            ...
            case 'show_subscriptions':
                ...
                if (! $forum_user['is_guest'] && $forum_config['o_show_dot'] == '1') {
                    ...
                    ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_subscriptions_has_posted')) ? eval($hook) : null;
                }
                
                $url_type = $forum_url['search_subscriptions'];
                $search_id = $value;
                
                ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_subscriptions')) ? eval($hook) : null;
            ...
            case 'show_forum_subscriptions':
                ...
                ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_forum_subscriptions')) ? eval($hook) : null;
            ...
            case 'show_unanswered':
                ...
                if (! $forum_user['is_guest'] && $forum_config['o_show_dot'] == '1') {
                    ...
                    ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_unanswered_topics_has_posted')) ? eval($hook) : null;
                }
                
                $url_type = $forum_url['search_unanswered'];
                
                ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_qr_get_unanswered')) ? eval($hook) : null;
                break;
        }
        
        ($hook = ForumFunction::get_hook('sf_fn_generate_action_search_query_end')) ? eval($hook) : null;
    ...
    function get_search_results($query, &$search_set)
    {
        ...
        $return = ($hook = ForumFunction::get_hook('sf_fn_get_search_results_start')) ? eval($hook) : null;
        if ($return !== null)
            return $return;
        ...
        $return = ($hook = ForumFunction::get_hook('sf_fn_get_search_results_end')) ? eval($hook) : null;
        if ($return !== null)
            return $return;
    ...
    function no_search_results($action = 'search')
    {
        ...
        $forum_page['search_again'] = '<a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['Perform new search'] . '</a>';
        
        $return = ($hook = ForumFunction::get_hook('sf_fn_no_search_results_start')) ? eval($hook) : null;
        if ($return !== null)
            return;
        
        switch ($action) {
            case 'show_new':
                ForumFunction::message($lang_search['No new posts'], $forum_page['search_again'], $lang_search['Topics with new']);
                break;
            
            case 'show_recent':
                ForumFunction::message($lang_search['No recent posts'], $forum_page['search_again'], $lang_search['Recently active topics']);
                break;
            
            case 'show_user_posts':
                ForumFunction::message($lang_search['No user posts'], $forum_page['search_again'], $lang_search['Posts by user']);
                break;
            
            case 'show_user_topics':
                ForumFunction::message($lang_search['No user topics'], $forum_page['search_again'], $lang_search['Topics by user']);
                break;
            
            case 'show_subscriptions':
                ForumFunction::message($lang_search['No subscriptions'], $forum_page['search_again'], $lang_search['Subscriptions']);
                break;
            
            case 'show_forum_subscriptions':
                ForumFunction::message($lang_search['No forum subscriptions'], $forum_page['search_again'], $lang_search['Forum subscriptions']);
                break;
            
            case 'show_unanswered':
                ForumFunction::message($lang_search['No unanswered'], $forum_page['search_again'], $lang_search['Unanswered topics']);
                break;
            
            default:
                ForumFunction::message($lang_search['No hits'], $forum_page['search_again'], $lang_search['Search results']);
                break;
        }
    }
    ...
    function generate_search_crumbs($action = null)
    {
        ...
        $return = ($hook = ForumFunction::get_hook('sf_fn_generate_search_crumbs_start')) ? eval($hook) : null;
        if ($return !== null)
            return;
        
        switch ($action) {
            case 'show_new':
                get_container()['breadcrumbs']->addCrumb($lang_search['Topics with new']);
                $forum_page['items_info'] = ForumFunction::generate_items_info($lang_search['Topics found'], ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['defined_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['User defined search'] . '</a></span>';
                $forum_page['main_foot_options']['mark_all'] = '<span' . (empty($forum_page['main_foot_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['mark_read'], ForumFunction::generate_form_token('markread' . $forum_user['id'])) . '">' . $lang_common['Mark all as read'] . '</a></span>';
                
                // Add link for show all topics, not only new (updated)
                if ($search_id != - 1)
                    $forum_page['main_head_options']['show_all'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['forum'], $search_set[0]['forum_id']) . '">' . $lang_search['All Topics'] . '</a></span>';
                
                break;
            
            case 'show_recent':
                $forum_page['crumbs'][] = $lang_search['Recently active topics'];
                get_container()['breadcrumbs']->addCrumb($lang_search['Recently active topics']);
                $forum_page['items_info'] = ForumFunction::generate_items_info($lang_search['Topics found'], ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['defined_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['User defined search'] . '</a></span>';
                break;
            ...
            case 'show_unanswered':
                ...
                get_container()['breadcrumbs']->addCrumb($lang_search['Unanswered topics']);
                $forum_page['items_info'] = ForumFunction::generate_items_info($lang_search['Topics found'], ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['defined_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['User defined search'] . '</a></span>';
                break;
            ...
            case 'show_user_posts':
                ...
                get_container()['breadcrumbs']->addCrumb(sprintf($lang_search['Posts by'], $search_set[0]['pposter'], ($forum_page['start_from'] + 1), $num_hits));
                $forum_page['items_info'] = ForumFunction::generate_items_info($lang_search['Posts found'], ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['user_topics'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search_user_topics'], $search_id) . '">' . sprintf($lang_search['Topics by'], ForumFunction::forum_htmlencode($search_set[0]['pposter'])) . '</a></span>';
                $forum_page['main_head_options']['defined_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['User defined search'] . '</a></span>';
                break;
            
            case 'show_user_topics':
                $forum_page['crumbs'][] = sprintf($lang_search['Topics by'], $search_set[0]['poster']);
                get_container()['breadcrumbs']->addCrumb(sprintf($lang_search['Topics by'], $search_set[0]['poster']));
                $forum_page['items_info'] = ForumFunction::generate_items_info($lang_search['Topics found'], ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['user_posts'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search_user_posts'], $search_id) . '">' . sprintf($lang_search['Posts by'], ForumFunction::forum_htmlencode($search_set[0]['poster'])) . '</a></span>';
                $forum_page['main_head_options']['defined_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['User defined search'] . '</a></span>';
                break;
            
            case 'show_subscriptions':
                $forum_page['crumbs'][] = $lang_search['Subscriptions'];
                get_container()['breadcrumbs']->addCrumb($lang_search['Subscriptions']);
                $forum_page['items_info'] = ForumFunction::generate_items_info($lang_search['Topics found'], ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['defined_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['User defined search'] . '</a></span>';
                break;
            ...
            case 'show_forum_subscriptions':
                ...
                get_container()['breadcrumbs']->addCrumb($lang_search['Forum subscriptions']);
                $forum_page['items_info'] = ForumFunction::generate_items_info($lang_search['Forums found'], ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['defined_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['User defined search'] . '</a></span>';
                break;
            
            default:
                $forum_page['crumbs'][] = $lang_search['Search results'];
                get_container()['breadcrumbs']->addCrumb($forum_page['crumbs'][] = $lang_search['Search results']);
                $forum_page['items_info'] = ForumFunction::generate_items_info((($show_as == 'topics') ? $lang_search['Topics found'] : $lang_search['Posts found']), ($forum_page['start_from'] + 1), $num_hits);
                $forum_page['main_head_options']['new_search'] = '<span' . (empty($forum_page['main_head_options']) ? ' class="first-item"' : '') . '><a href="' . ForumFunction::forum_link($forum_url['search']) . '">' . $lang_search['Perform new search'] . '</a></span>';
                break;
        }
    }
    ...
    function validate_search_action($action)
    {
        ...
        $valid_actions = array(
            ...
            'show_user_topics',
            'show_subscriptions',
            'show_forum_subscriptions',
            'show_unanswered'
        );
        
        $return = ($hook = ForumFunction::get_hook('sf_fn_validate_actions_start')) ? eval($hook) : null;
        if ($return !== null)
            return $return;
