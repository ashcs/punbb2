...
class DBLayer
{
	...
	function __construct($db_host, $db_username, $db_password, $db_name, $db_prefix, $p_connect)
	{
		...
		if (!$this->link_id)
		 ForumFunction::error('Unable to connect to PostgreSQL server.', __FILE__, __LINE__);
	...
	function add_field($table_name, $field_name, $field_type, $allow_null, $default_value = null, $after_field = null, $no_prefix = false)
	{
		...
		$this->query('ALTER TABLE '.($no_prefix ? '' : $this->prefix).$table_name.' ADD '.$field_name.' '.$field_type) or ForumFunction::error(__FILE__, __LINE__);
		...
		if ($default_value !== null)
		{
			...
			$this->query('ALTER TABLE '.($no_prefix ? '' : $this->prefix).$table_name.' ALTER '.$field_name.' SET DEFAULT '.$default_value) or ForumFunction::error(__FILE__, __LINE__);
			$this->query('UPDATE '.($no_prefix ? '' : $this->prefix).$table_name.' SET '.$field_name.'='.$default_value) or ForumFunction::error(__FILE__, __LINE__);
		}

		if (!$allow_null)
			$this->query('ALTER TABLE '.($no_prefix ? '' : $this->prefix).$table_name.' ALTER '.$field_name.' SET NOT NULL') or ForumFunction::error(__FILE__, __LINE__);
	}
	...
	function alter_field($table_name, $field_name, $field_type, $allow_null, $default_value = null, $after_field = null, $no_prefix = false)
	{
		...
		$this->query('UPDATE '.($no_prefix ? '' : $this->prefix).$table_name.' SET tmp_'.$field_name.' = '.$field_name) or ForumFunction::error(__FILE__, __LINE__);
		$this->drop_field($table_name, $field_name, $no_prefix);
		$this->query('ALTER TABLE '.($no_prefix ? '' : $this->prefix).$table_name.' RENAME COLUMN tmp_'.$field_name.' TO '.$field_name) or ForumFunction::error(__FILE__, __LINE__);
	...
	function drop_field($table_name, $field_name, $no_prefix = false)
	{
		...
		$this->query('ALTER TABLE '.($no_prefix ? '' : $this->prefix).$table_name.' DROP '.$field_name) or ForumFunction::error(__FILE__, __LINE__);
	}
