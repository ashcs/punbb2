...
if ($forum_page['num_users'] > 0)
    $forum_page['items_info'] = \Punbb\ForumFunction::generate_items_info((($forum_page['users_searched']) ? $lang_ul['Users found'] : $lang_ul['Users']), ($forum_page['start_from'] + 1), $forum_page['num_users']);
else
    $forum_page['items_info'] = $lang_ul['Users'];

// Generate paging links
$forum_page['page_post']['paging'] = '<p class="paging"><span class="pages">' . $lang_common['Pages'] . '</span> ' . \Punbb\ForumFunction::paginate($forum_page['num_pages'], $forum_page['page'], $forum_url['users_browse'], $lang_common['Paging separator'], array(
    $forum_page['show_group'],
    $forum_page['sort_by'],
    $forum_page['sort_dir'],
    ($forum_page['username'] != '') ? urlencode($forum_page['username']) : '-'
)) . '</p>';

// Navigation links for header and page numbering for title/meta description
if ($forum_page['page'] < $forum_page['num_pages']) {
    $forum_page['nav']['last'] = '<link rel="last" href="' . \Punbb\ForumFunction::forum_sublink($forum_url['users_browse'], $forum_url['page'], $forum_page['num_pages'], array(
        $forum_page['show_group'],
        $forum_page['sort_by'],
        $forum_page['sort_dir'],
        ($forum_page['username'] != '') ? urlencode($forum_page['username']) : '-'
    )) . '" title="' . $lang_common['Page'] . ' ' . $forum_page['num_pages'] . '" />';
    $forum_page['nav']['next'] = '<link rel="next" href="' . \Punbb\ForumFunction::forum_sublink($forum_url['users_browse'], $forum_url['page'], ($forum_page['page'] + 1), array(
        $forum_page['show_group'],
        $forum_page['sort_by'],
        $forum_page['sort_dir'],
        ($forum_page['username'] != '') ? urlencode($forum_page['username']) : '-'
...
if ($forum_page['page'] > 1) {
    $forum_page['nav']['prev'] = '<link rel="prev" href="' . \Punbb\ForumFunction::forum_sublink($forum_url['users_browse'], $forum_url['page'], ($forum_page['page'] - 1), array(
        $forum_page['show_group'],
        $forum_page['sort_by'],
        $forum_page['sort_dir'],
        ($forum_page['username'] != '') ? urlencode($forum_page['username']) : '-'
    )) . '" title="' . $lang_common['Page'] . ' ' . ($forum_page['page'] - 1) . '" />';
    $forum_page['nav']['first'] = '<link rel="first" href="' . \Punbb\ForumFunction::forum_link($forum_url['users_browse'], array(
        $forum_page['show_group'],
        $forum_page['sort_by'],
        $forum_page['sort_dir'],
        ($forum_page['username'] != '') ? urlencode($forum_page['username']) : '-'
...
if (empty($_GET))
    ...
    $forum_page['main_head_options'] = array(
        'new_search' => '<span' . (empty($forum_page['main_foot_options']) ? ' class="first-item"' : '') . '><a href="' . \Punbb\ForumFunction::forum_link($forum_url['users']) . '">' . $lang_ul['Perform new search'] . '</a></span>'
    );
...
($hook = \Punbb\ForumFunction::get_hook('ul_pre_header_load')) ? eval($hook) : null;

($hook = \Punbb\ForumFunction::get_hook('ul_main_output_start')) ? eval($hook) : null;
...
?>
	...
	<div class="main-content main-frm">
		...
		<div class="frm-form">
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_search_fieldset_start')) ? eval($hook) : null; ?>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><strong><?php echo $lang_ul['User find legend'] ?></strong></legend>
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_pre_username')) ? eval($hook) : null; ?>
<?php if ($forum_user['g_search_users'] == '1'): ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						...
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="username" value="<?php echo \Punbb\ForumFunction::forum_htmlencode($forum_page['username']) ?>" size="35" maxlength="25" /></span>
					</div>
				</div>
<?php endif; ?>
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_pre_group_select')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
...
($hook = \Punbb\ForumFunction::get_hook('ul_search_new_group_option')) ? eval($hook) : null;
...
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_pre_sort_by')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box select">
...
<?php endif; ($hook = \Punbb\ForumFunction::get_hook('ul_new_sort_by_option')) ? eval($hook) : null; ?>
						</select></span>
					</div>
				</div>
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_pre_sort_order_fieldset')) ? eval($hook) : null; ?>
				<fieldset class="mf-set set<?php echo ++$forum_page['item_count'] ?>">
					<legend><span><?php echo $lang_ul['User sort order'] ?></span></legend>
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_pre_sort_order')) ? eval($hook) : null; ?>
...
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_pre_sort_order_fieldset_end')) ? eval($hook) : null; ?>
				</fieldset>
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_pre_search_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php ($hook = \Punbb\ForumFunction::get_hook('ul_search_fieldset_end')) ? eval($hook) : null; ?>
...
{
	($hook = \Punbb\ForumFunction::get_hook('ul_results_pre_header')) ? eval($hook) : null;
	...
	($hook = \Punbb\ForumFunction::get_hook('ul_results_pre_header_output')) ? eval($hook) : null;
...
<?php
	...
	{
		($hook = \Punbb\ForumFunction::get_hook('ul_results_row_pre_data')) ? eval($hook) : null;

		$forum_page['table_row'] = array();
		$forum_page['table_row']['username'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.\Punbb\ForumFunction::forum_link($forum_url['user'], $user_data['id']).'">'.\Punbb\ForumFunction::forum_htmlencode($user_data['username']).'</a></td>';
		$forum_page['table_row']['title'] = '<td class="tc'.count($forum_page['table_row']).'">'.\Punbb\ForumFunction::get_title($user_data).'</td>';

		if ($forum_page['show_post_count'])
			$forum_page['table_row']['posts'] = '<td class="tc'.count($forum_page['table_row']).'">'.\Punbb\ForumFunction::forum_number_format($user_data['num_posts']).'</td>';

		$forum_page['table_row']['registered'] = '<td class="tc'.count($forum_page['table_row']).'">'.\Punbb\ForumFunction::format_time($user_data['registered'], 1).'</td>';

		++$forum_page['item_count'];

		($hook = \Punbb\ForumFunction::get_hook('ul_results_row_pre_data_output')) ? eval($hook) : null;
...
($hook = \Punbb\ForumFunction::get_hook('ul_end')) ? eval($hook) : null;
