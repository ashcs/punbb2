...
<?php
	...
	if (!empty($errors))
	{
		...
		($hook = \Punbb\ForumFunction::get_hook('li_forgot_pass_pre_new_password_errors')) ? eval($hook) : null;
...
<?php ($hook = \Punbb\ForumFunction::get_hook('li_forgot_pass_pre_group')) ? eval($hook) : null; ?>
			<div class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
<?php ($hook = \Punbb\ForumFunction::get_hook('li_forgot_pass_pre_email')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text required">
						...
						<span class="fld-input"><input id="fld<?php echo $forum_page['fld_count'] ?>" type="email" name="req_email" value="<?php if (isset($_POST['req_email'])) echo \Punbb\ForumFunction::forum_htmlencode($_POST['req_email']); ?>" size="35" maxlength="80" required spellcheck="false" /></span>
					</div>
				</div>
<?php ($hook = \Punbb\ForumFunction::get_hook('li_forgot_pass_pre_group_end')) ? eval($hook) : null; ?>
			</div>
<?php ($hook = \Punbb\ForumFunction::get_hook('li_forgot_pass_group_end')) ? eval($hook) : null; ?>
