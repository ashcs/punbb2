...
($hook = ForumFunction::get_hook('re_rewrite_rules')) ? eval($hook) : null;
...
if (empty($rewritten_url))
{
	...
	($hook = ForumFunction::get_hook('re_page_not_found')) ? eval($hook) : null;

 ForumFunction::error('Page Not found (Error 404):<br />The requested page <em>'.ForumFunction::forum_htmlencode($request_uri).'</em> could not be found.');
}
