...
$forum_page['frm_info'] = array(
	'<li><span>'.$lang_delete['Forum'].':<strong> '.ForumFunction::forum_htmlencode($cur_post['forum_name']).'</strong></span></li>',
	'<li><span>'.$lang_delete['Topic'].':<strong> '.ForumFunction::forum_htmlencode($cur_post['subject']).'</strong></span></li>'
);

// Generate the post heading
$forum_page['post_ident'] = array();
$forum_page['post_ident']['byline'] = '<span class="post-byline">'.sprintf((($cur_post['is_topic']) ? $lang_delete['Topic byline'] : $lang_delete['Reply byline']), '<strong>'.ForumFunction::forum_htmlencode($cur_post['poster']).'</strong>').'</span>';
$forum_page['post_ident']['link'] = '<span class="post-link"><a class="permalink" href="'.ForumFunction::forum_link($forum_url['post'], $cur_post['tid']).'">'.ForumFunction::format_time($cur_post['posted']).'</a></span>';
...
($hook = ForumFunction::get_hook('dl_main_output_start')) ? eval($hook) : null;
...
<?php ($hook = ForumFunction::get_hook('dl_pre_post_display')) ? eval($hook) : null; ?>
		<div class="post singlepost">
			<div class="posthead">
				<h3 class="hn post-ident"><?php echo implode(' ', $forum_page['post_ident']) ?></h3>
<?php ($hook = ForumFunction::get_hook('dl_new_post_head_option')) ? eval($hook) : null; ?>
			</div>
			<div class="postbody">
...
<?php ($hook = ForumFunction::get_hook('dl_new_post_entry_data')) ? eval($hook) : null; ?>
				</div>
			</div>
		</div>
...
<?php ($hook = ForumFunction::get_hook('dl_pre_confirm_delete_fieldset')) ? eval($hook) : null; ?>
...
<?php ($hook = ForumFunction::get_hook('dl_pre_confirm_delete_checkbox')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box checkbox">
						...
						<label for="fld<?php echo $forum_page['fld_count'] ?>"><span><?php echo $lang_delete['Please confirm'] ?></span> <?php printf(((($cur_post['is_topic'])) ? $lang_delete['Delete topic label'] : $lang_delete['Delete post label']), ForumFunction::forum_htmlencode($cur_post['poster']), ForumFunction::format_time($cur_post['posted'])) ?></label>
					</div>
				</div>
<?php ($hook = ForumFunction::get_hook('dl_pre_confirm_delete_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php ($hook = ForumFunction::get_hook('dl_confirm_delete_fieldset_end')) ? eval($hook) : null; ?>
...
($hook = ForumFunction::get_hook('dl_end')) ? eval($hook) : null;
