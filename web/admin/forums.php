...
($hook = ForumFunction::get_hook('afo_start')) ? eval($hook) : null;
...
if (isset($_POST['add_forum']))
{
	...
	$forum_name = ForumFunction::forum_trim($_POST['forum_name']);
	$position = intval($_POST['position']);

	($hook = ForumFunction::get_hook('afo_add_forum_form_submitted')) ? eval($hook) : null;

	if ($forum_name == '')
		ForumFunction::message($lang_admin_forums['Must enter forum message']);
	...
	($hook = ForumFunction::get_hook('afo_add_forum_qr_validate_category_id')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('afo_add_forum_qr_add_forum')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('afo_add_forum_pre_redirect')) ? eval($hook) : null;

	ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_forums']), $lang_admin_forums['Forum added']);
}
...
else if (isset($_GET['del_forum']))
{
	...
	if (isset($_POST['del_forum_cancel']))
		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_forums']), $lang_admin_common['Cancel redirect']);

	($hook = ForumFunction::get_hook('afo_del_forum_form_submitted')) ? eval($hook) : null;
	...
	if (isset($_POST['del_forum_comply']))	// Delete a forum with all posts
	{
		...
		($hook = ForumFunction::get_hook('afo_del_forum_qr_delete_forum')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		($hook = ForumFunction::get_hook('afo_del_forum_qr_delete_forum_perms')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		($hook = ForumFunction::get_hook('afo_del_forum_qr_delete_forum_subscriptions')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		($hook = ForumFunction::get_hook('afo_del_forum_pre_redirect')) ? eval($hook) : null;

		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_forums']), $lang_admin_forums['Forum deleted']);
	}
	else	// If the user hasn't confirmed the delete
	{
		...
		($hook = ForumFunction::get_hook('afo_del_forum_qr_get_forum_name')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		$forum_page['crumbs'] = array(
			...
			array($lang_admin_common['Start'], ForumFunction::forum_link($forum_url['admin_index'])),
			array($lang_admin_common['Forums'], ForumFunction::forum_link($forum_url['admin_forums'])),
			$lang_admin_forums['Delete forum']
		);

		($hook = ForumFunction::get_hook('afo_del_forum_pre_header_load')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('afo_del_forum_output_start')) ? eval($hook) : null;

?>
	<div class="main-subhead">
		<h2 class="hn"><span><?php printf($lang_admin_forums['Confirm delete forum'], ForumFunction::forum_htmlencode($forum_name)) ?></span></h2>
	</div>
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo ForumFunction::forum_link($forum_url['admin_forums']) ?>?del_forum=<?php echo $forum_to_delete ?>">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token(ForumFunction::forum_link($forum_url['admin_forums']).'?del_forum='.$forum_to_delete) ?>" />
			</div>
...
<?php
		...
		($hook = ForumFunction::get_hook('afo_del_forum_end')) ? eval($hook) : null;
...
else if (isset($_POST['update_positions']))
{
	...
	($hook = ForumFunction::get_hook('afo_update_positions_form_submitted')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('afo_update_positions_qr_get_forums')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	while ($cur_forum = $forum_db->fetch_assoc($result))
	{
		...
		if (isset($positions[$cur_forum['id']]))
		{
			...
			if ($new_disp_position < 0)
				ForumFunction::message($lang_admin_forums['Must be integer']);
			...
			if ($cur_forum['disp_position'] != $new_disp_position)
			{
				...
				($hook = ForumFunction::get_hook('afo_update_positions_qr_update_forum_position')) ? eval($hook) : null;
				$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
			}
		}
	}
	...
	($hook = ForumFunction::get_hook('afo_update_positions_pre_redirect')) ? eval($hook) : null;

	ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_forums']), $lang_admin_forums['Forums updated']);
}
...
else if (isset($_GET['edit_forum']))
{
	...
	($hook = ForumFunction::get_hook('afo_edit_forum_selected')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('afo_edit_forum_qr_get_forum_details')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	if (isset($_POST['save']))
	{
		...
		$forum_name = ForumFunction::forum_trim($_POST['forum_name']);
		$forum_desc = ForumFunction::forum_linebreaks(ForumFunction::forum_trim($_POST['forum_desc']));
		$cat_id = intval($_POST['cat_id']);
		$sort_by = intval($_POST['sort_by']);
		$redirect_url = isset($_POST['redirect_url']) && $cur_forum['num_topics'] == 0 ? ForumFunction::forum_trim($_POST['redirect_url']) : null;

		($hook = ForumFunction::get_hook('afo_save_forum_form_submitted')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('afo_save_forum_qr_update_forum')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		if (isset($_POST['read_forum_old']))
		{
			...
			($hook = ForumFunction::get_hook('afo_save_forum_qr_get_groups')) ? eval($hook) : null;
			$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
			while ($cur_group = $forum_db->fetch_assoc($result))
			{
				...
				($hook = ForumFunction::get_hook('afo_save_forum_pre_perms_compare')) ? eval($hook) : null;
				...
				if ($perms_new !== $perms_old)
				{
					...
					if ($perms_new === $perms_default)
					{
						...
						($hook = ForumFunction::get_hook('afo_save_forum_qr_delete_group_forum_perms')) ? eval($hook) : null;
						$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
					}
					else
					{
						...
						($hook = ForumFunction::get_hook('afo_save_forum_qr_update_forum_perms')) ? eval($hook) : null;
						$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
						if (!$forum_db->affected_rows())
						{
							...
							($hook = ForumFunction::get_hook('afo_save_forum_qr_add_forum_perms')) ? eval($hook) : null;
							$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
						}
					}
				}
		...
		($hook = ForumFunction::get_hook('afo_save_forum_pre_redirect')) ? eval($hook) : null;

		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_forums_forum'], $forum_id), $lang_admin_forums['Forum updated']);
	}
	else if (isset($_POST['revert_perms']))
	{
		($hook = ForumFunction::get_hook('afo_revert_perms_form_submitted')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('afo_revert_perms_qr_revert_forum_perms')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		($hook = ForumFunction::get_hook('afo_revert_perms_pre_redirect')) ? eval($hook) : null;

		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_forums']).'?edit_forum='.$forum_id, $lang_admin_forums['Permissions reverted']);
	}
	...
	$forum_page['form_info']['groups'] = '<li><span>'. sprintf($lang_admin_forums['Forum perms groups info'], '<a href="'.ForumFunction::forum_link($forum_url['admin_groups']).'">'.$lang_admin_forums['User groups'].'</a>').'</span></li>';
	...
	($hook = ForumFunction::get_hook('afo_edit_forum_pre_header_load')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('afo_edit_forum_output_start')) ? eval($hook) : null;

?>
	<div class="main-subhead">
		<h2 class="hn"><span><?php printf($lang_admin_forums['Edit forum head'], ForumFunction::forum_htmlencode($cur_forum['forum_name'])) ?></span></h2>
	</div>
	<div class="main-content main-frm">
		<form method="post" class="frm-form" accept-charset="utf-8" action="<?php echo ForumFunction::forum_link($forum_url['admin_forums']) ?>?edit_forum=<?php echo $forum_id ?>">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token(ForumFunction::forum_link($forum_url['admin_forums']).'?edit_forum='.$forum_id) ?>" />
			</div>
			<div class="content-head">
				<h3 class="hn"><span><?php echo $lang_admin_forums['Edit forum details head'] ?></span></h3>
			</div>
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_details_fieldset')) ? eval($hook) : null; ?>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><strong><?php echo $lang_admin_forums['Edit forum details legend'] ?></strong></legend>
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_forum_name')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_admin_forums['Forum name'] ?></span></label><br />
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="forum_name" size="35" maxlength="80" value="<?php echo ForumFunction::forum_htmlencode($cur_forum['forum_name']) ?>" required /></span>
					</div>
				</div>
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_forum_descrip')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea">
						...
						<div class="txt-input"><span class="fld-input"><textarea id="fld<?php echo $forum_page['fld_count'] ?>" name="forum_desc" rows="3" cols="50"><?php echo ForumFunction::forum_htmlencode($cur_forum['forum_desc']) ?></textarea></span></div>
					</div>
				</div>
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_forum_cat')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
...
<?php
	...
	($hook = ForumFunction::get_hook('afo_edit_forum_qr_get_categories')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	while ($cur_cat = $forum_db->fetch_assoc($result))
	{
		$selected = ($cur_cat['id'] == $cur_forum['cat_id']) ? ' selected="selected"' : '';
		echo "\t\t\t\t\t\t\t\t".'<option value="'.$cur_cat['id'].'"'.$selected.'>'.ForumFunction::forum_htmlencode($cur_cat['cat_name']).'</option>'."\n";
	}
...
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_forum_sort_by')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box select">
...
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_modify_sort_by')) ? eval($hook) : null; ?>						</select></span>
					</div>
				</div>
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_forum_redirect_url')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_admin_forums['Redirect URL'] ?></span></label><br />
						<span class="fld-input"><?php echo ($cur_forum['num_topics']) ? '<input type="url" id="fld'.$forum_page['fld_count'].'" name="redirect_url" size="45" maxlength="100" value="'.$lang_admin_forums['Only for empty forums'].'" disabled="disabled" />' : '<input type="text" id="fld'.$forum_page['fld_count'].'" name="redirect_url" size="35" maxlength="100" value="'.ForumFunction::forum_htmlencode($cur_forum['redirect_url']).'" />' ?></span>
					</div>
				</div>
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_details_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php

($hook = ForumFunction::get_hook('afo_edit_forum_details_fieldset_end')) ? eval($hook) : null;

// Reset fieldset counter
$forum_page['group_count'] = $forum_page['item_count'] = 0;

($hook = ForumFunction::get_hook('afo_edit_forum_pre_permissions_part')) ? eval($hook) : null;
...
<?php
	...
	($hook = ForumFunction::get_hook('afo_qr_get_forum_perms')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
...
($hook = ForumFunction::get_hook('afo_edit_forum_pre_cur_group_permissions_fieldset')) ? eval($hook) : null;

?>
				<fieldset class="mf-set set<?php echo ++$forum_page['item_count'] ?>">
					<legend><span><?php echo ForumFunction::forum_htmlencode($cur_perm['g_title']) ?></span></legend>
					<div class="mf-box mf-yesno">
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_cur_group_read_forum_permission')) ? eval($hook) : null; ?>
						<div class="mf-item">
...
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_cur_group_post_replies_permission')) ? eval($hook) : null; ?>
						<div class="mf-item">
...
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_cur_group_post_topics_permission')) ? eval($hook) : null; ?>
						<div class="mf-item">
...
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_post_cur_group_post_topics_permission')) ? eval($hook) : null; ?>
					</div>
<?php ($hook = ForumFunction::get_hook('afo_edit_forum_pre_cur_group_permissions_fieldset_end')) ? eval($hook) : null; ?>
				</fieldset>
<?php

		($hook = ForumFunction::get_hook('afo_edit_forum_cur_group_permissions_fieldset_end')) ? eval($hook) : null;
...
<?php
	...
	($hook = ForumFunction::get_hook('afo_edit_forum_end')) ? eval($hook) : null;
...
($hook = ForumFunction::get_hook('afo_pre_header_load')) ? eval($hook) : null;
...
($hook = ForumFunction::get_hook('afo_main_output_start')) ? eval($hook) : null;
...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo ForumFunction::forum_link($forum_url['admin_forums']) ?>?action=adddel">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token(ForumFunction::forum_link($forum_url['admin_forums']).'?action=adddel') ?>" />
			</div>
<?php ($hook = ForumFunction::get_hook('afo_pre_add_forum_fieldset')) ? eval($hook) : null; ?>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><strong><?php echo $lang_admin_forums['Add forum legend'] ?></strong></legend>
<?php ($hook = ForumFunction::get_hook('afo_pre_new_forum_name')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
...
<?php ($hook = ForumFunction::get_hook('afo_pre_new_forum_position')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
...
<?php ($hook = ForumFunction::get_hook('afo_pre_new_forum_cat')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box select">
...
<?php
	...
	($hook = ForumFunction::get_hook('afo_qr_get_categories')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	while ($cur_cat = $forum_db->fetch_assoc($result))
		echo "\t\t\t\t\t\t\t".'<option value="'.$cur_cat['id'].'">'.ForumFunction::forum_htmlencode($cur_cat['cat_name']).'</option>'."\n";

?>
						</select></span>
					</div>
				</div>
<?php ($hook = ForumFunction::get_hook('afo_pre_add_forum_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php ($hook = ForumFunction::get_hook('afo_add_forum_fieldset_end')) ? eval($hook) : null; ?>
			<div class="frm-buttons">
...
($hook = ForumFunction::get_hook('afo_qr_get_cats_and_forums')) ? eval($hook) : null;
$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo ForumFunction::forum_link($forum_url['admin_forums']) ?>?action=edit">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token(ForumFunction::forum_link($forum_url['admin_forums']).'?action=edit') ?>" />
			</div>
...
?>
			<div class="content-head">
				<h3 class="hn"><span><?php printf($lang_admin_forums['Forums in category'], ForumFunction::forum_htmlencode($cur_forum['cat_name'])) ?></span></h3>
			</div>
...
($hook = ForumFunction::get_hook('afo_pre_edit_cur_forum_fieldset')) ? eval($hook) : null;
...
?>
				<fieldset id="forum<?php echo $cur_forum['fid'] ?>" class="mf-set set<?php echo ++$forum_page['item_count'] ?><?php echo ($forum_page['item_count'] == 1) ? ' mf-head' : ' mf-extra' ?>">
					<legend><span><?php printf($lang_admin_forums['Edit or delete'], '<a href="'.ForumFunction::forum_link($forum_url['admin_forums']).'?edit_forum='.$cur_forum['fid'].'">'.$lang_admin_forums['Edit'].'</a>', '<a href="'.ForumFunction::forum_link($forum_url['admin_forums']).'?del_forum='.$cur_forum['fid'].'">'.$lang_admin_forums['Delete'].'</a>') ?></span></legend>
					<div class="mf-box">
<?php ($hook = ForumFunction::get_hook('afo_pre_edit_cur_forum_name')) ? eval($hook) : null; ?>
						<div class="mf-field mf-field1 forum-field">
							<span class="aslabel"><?php echo $lang_admin_forums['Forum name'] ?></span>
							<span class="fld-input"><?php echo ForumFunction::forum_htmlencode($cur_forum['forum_name']) ?></span>
						</div>
<?php ($hook = ForumFunction::get_hook('afo_pre_edit_cur_forum_position')) ? eval($hook) : null; ?>
						<div class="mf-field">
							<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_admin_forums['Position label'] ?></span></label><br />
...
<?php ($hook = ForumFunction::get_hook('afo_pre_edit_cur_forum_fieldset_end')) ? eval($hook) : null; ?>
				</fieldset>
<?php

		($hook = ForumFunction::get_hook('afo_edit_cur_forum_fieldset_end')) ? eval($hook) : null;
...
($hook = ForumFunction::get_hook('afo_end')) ? eval($hook) : null;
