...
($hook = ForumFunction::get_hook('aex_start')) ? eval($hook) : null;
...
if (!function_exists('xml_parser_create'))
	ForumFunction::message($lang_admin_ext['No XML support']);
...
{
	($hook = ForumFunction::get_hook('aex_install_selected')) ? eval($hook) : null;

	// User pressed the cancel button
	if (isset($_POST['install_cancel']))
		ForumFunction::redirect(ForumFunction::forum_link(isset($_GET['install']) ? $forum_url['admin_extensions_manage'] : $forum_url['admin_extensions_hotfixes']), $lang_admin_common['Cancel redirect']);
	...
	{
		$remote_file = ForumFunction::get_remote_file('http://punbb.informer.com/update/manifest/'.$id.'.xml', 16);
	...
	if (!empty($errors)) {
		...
		$msg_errors =
			'<div class="ct-box error-box"><h2 class="warn hn">'
				...
				. '</ul>'
			. '</div>';
		ForumFunction::message(isset($_GET['install'])? $msg_errors : $lang_admin_ext['Hotfix download failed']);
	}

	// Get core amd major versions
	if (!defined('FORUM_DISABLE_EXTENSIONS_VERSION_CHECK'))
	{
		list($forum_version_core, $forum_version_major) = explode('.', ForumFunction::clean_version($forum_config['o_cur_version']));
		list($extension_maxtestedon_version_core, $extension_maxtestedon_version_major) = explode('.', ForumFunction::clean_version($ext_data['extension']['maxtestedon']));
		...
		if (version_compare($forum_version_core.'.'.$forum_version_major, $extension_maxtestedon_version_core.'.'.$extension_maxtestedon_version_major, '>'))
			ForumFunction::message($lang_admin_ext['Maxtestedon error']);
	}
	...
	($hook = ForumFunction::get_hook('aex_install_check_dependencies')) ? eval($hook) : null;
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_common['Extensions'], ForumFunction::forum_link($forum_url['admin_extensions_manage'])),
		array((strpos($id, 'hotfix_') === 0) ? $lang_admin_common['Manage hotfixes'] : $lang_admin_common['Manage extensions'], (strpos($id, 'hotfix_') === 0) ? ForumFunction::forum_link($forum_url['admin_extensions_hotfixes']) : ForumFunction::forum_link($forum_url['admin_extensions_manage'])),
	...
	{
		($hook = ForumFunction::get_hook('aex_install_comply_form_submitted')) ? eval($hook) : null;
		...
		$uninstall_code = (isset($ext_data['extension']['uninstall']) && ForumFunction::forum_trim($ext_data['extension']['uninstall']) != '') ? '\''.$forum_db->escape(ForumFunction::forum_trim($ext_data['extension']['uninstall'])).'\'' : 'NULL';

		// Is there an uninstall note to store in the db?
		$uninstall_note = 'NULL';
		foreach ($ext_data['extension']['note'] as $cur_note)
		{
			if ($cur_note['attributes']['type'] == 'uninstall' && ForumFunction::forum_trim($cur_note['content']) != '')
				$uninstall_note = '\''.$forum_db->escape(ForumFunction::forum_trim($cur_note['content'])).'\'';
		}
		...
		($hook = ForumFunction::get_hook('aex_install_comply_qr_get_current_ext_version')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		if (!is_null($ext_version) && $ext_version !== false)
		{
			...
			if (isset($ext_data['extension']['install']) && ForumFunction::forum_trim($ext_data['extension']['install']) != '')
			...
			($hook = ForumFunction::get_hook('aex_install_comply_qr_update_ext')) ? eval($hook) : null;
			...
			($hook = ForumFunction::get_hook('aex_install_comply_qr_update_ext_delete_hooks')) ? eval($hook) : null;
			$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		}
		else
		{
			...
			($hook = ForumFunction::get_hook('aex_install_comply_qr_add_ext')) ? eval($hook) : null;
			$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		}
			...
			foreach ($ext_data['extension']['hooks']['hook'] as $ext_hook)
			{
				...
				foreach ($cur_hooks as $cur_hook)
				{
					$query = array(
						...
						'VALUES'	=> '\''.$forum_db->escape(ForumFunction::forum_trim($cur_hook)).'\', \''.$forum_db->escape($id).'\', \''.$forum_db->escape(ForumFunction::forum_trim($ext_hook['content'])).'\', '.time().', '.(isset($ext_hook['attributes']['priority']) ? $ext_hook['attributes']['priority'] : 5)
					);

					($hook = ForumFunction::get_hook('aex_install_comply_qr_add_hook')) ? eval($hook) : null;
					$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
				}
			}
		}
		...
		// Empty the PHP cache
		ForumFunction::forum_clear_cache();
		...
		if (!empty($notices))
		{
			($hook = ForumFunction::get_hook('aex_install_notices_pre_header_load')) ? eval($hook) : null;
			...
			($hook = ForumFunction::get_hook('aex_install_notices_output_start')) ? eval($hook) : null;

?>
	<div class="main-subhead">
		<h2 class="hn"><span><?php echo end($forum_page['crumbs']) ?> "<?php echo ForumFunction::forum_htmlencode($ext_data['extension']['title']) ?>"</span></h2>
	</div>
...
?>
			...
			<p><a href="<?php echo ForumFunction::forum_link($forum_url['admin_extensions_manage']) ?>"><?php echo $lang_admin_common['Manage extensions'] ?></a></p>
		</div>
	</div>
<?php

			($hook = ForumFunction::get_hook('aex_install_notices_end')) ? eval($hook) : null;
		...
		else
		{
			...
			($hook = ForumFunction::get_hook('aex_install_comply_pre_redirect')) ? eval($hook) : null;

			if (strpos($id, 'hotfix_') === 0)
				ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_extensions_hotfixes']), $lang_admin_ext['Hotfix installed']);
			else
				ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_extensions_manage']), $lang_admin_ext['Extension installed']);
		}
	}


	($hook = ForumFunction::get_hook('aex_install_pre_header_load')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('aex_install_output_start')) ? eval($hook) : null;
?>
	<div class="main-subhead">
		<h2 class="hn"><span><?php echo end($forum_page['crumbs']) ?> "<?php echo ForumFunction::forum_htmlencode($ext_data['extension']['title']) ?>"</span></h2>
	</div>
...
<?php
    ...
    if (!empty($errors))
    {
        ...
        foreach ($errors as $cur_error)
            ...
            ($hook = ForumFunction::get_hook('aex_install_ext_pre_errors')) ? eval($hook) : null;
...
?>
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo $base_url.'/admin/extensions.php'.(isset($_GET['install']) ? '?install=' : '?install_hotfix=').$id ?>">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token($base_url.'/admin/extensions.php'.(isset($_GET['install']) ? '?install=' : '?install_hotfix=').$id) ?>" />
			</div>
			<div class="ct-group data-group">
				<div class="ct-set data-set set1">
					<div class="ct-box data-box">
						<h3 class="ct-legend hn"><span><?php echo ForumFunction::forum_htmlencode($ext_data['extension']['title']) ?></span></h3>
						<p><?php echo ((strpos($id, 'hotfix_') !== 0) ? sprintf($lang_admin_ext['Version'], $ext_data['extension']['version']) : $lang_admin_ext['Hotfix']) ?></p>
						<p><?php printf($lang_admin_ext['Extension by'], ForumFunction::forum_htmlencode($ext_data['extension']['author'])) ?></p>
						<p><?php echo ForumFunction::forum_htmlencode($ext_data['extension']['description']) ?></p>
					</div>
				</div>
			</div>
<?php
	...
	foreach ($ext_data['extension']['note'] as $cur_note)
	{
		if ($cur_note['attributes']['type'] == 'install')
			$form_warnings[] = '<li>'.ForumFunction::forum_htmlencode($cur_note['content']).'</li>';
	}

	if (version_compare(ForumFunction::clean_version($forum_config['o_cur_version']), ForumFunction::clean_version($ext_data['extension']['maxtestedon']), '>'))
...
<?php
	...
	($hook = ForumFunction::get_hook('aex_install_end')) ? eval($hook) : null;
...
else if (isset($_GET['uninstall']))
{
	...
	if (isset($_POST['uninstall_cancel']))
		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_extensions_manage']), $lang_admin_common['Cancel redirect']);

	($hook = ForumFunction::get_hook('aex_uninstall_selected')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('aex_uninstall_qr_get_extension')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	($hook = ForumFunction::get_hook('aex_uninstall_qr_check_dependencies')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	$dependency = $forum_db->fetch_assoc($result);

	if (!is_null($dependency) && $dependency !== false)
	{
		ForumFunction::message(sprintf($lang_admin_ext['Uninstall dependency'], $dependency['id']));
	}
	...
	{
		($hook = ForumFunction::get_hook('aex_uninstall_comply_form_submitted')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('aex_uninstall_comply_qr_uninstall_delete_hooks')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		($hook = ForumFunction::get_hook('aex_uninstall_comply_qr_delete_extension')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		if (!empty($notices))
		{
			($hook = ForumFunction::get_hook('aex_uninstall_notices_pre_header_load')) ? eval($hook) : null;
			...
			($hook = ForumFunction::get_hook('aex_uninstall_notices_output_start')) ? eval($hook) : null;

?>
	<div class="main-subhead">
		<h2 class="hn"><span><?php echo end($forum_page['crumbs']) ?> "<?php echo ForumFunction::forum_htmlencode($ext_data['title']) ?>"</span></h2>
	</div>
	<div class="main-content main-frm">
		<div class="ct-box info-box">
...
<?php
			...
			($hook = ForumFunction::get_hook('aex_uninstall_notices_end')) ? eval($hook) : null;
		...
		else
		{
			...
			($hook = ForumFunction::get_hook('aex_uninstall_comply_pre_redirect')) ? eval($hook) : null;

			if (strpos($id, 'hotfix_') === 0)
				ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_extensions_hotfixes']), $lang_admin_ext['Hotfix uninstalled']);
			else
				ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_extensions_manage']), $lang_admin_ext['Extension uninstalled']);
		}
	}
	else	// If the user hasn't confirmed the uninstall
	{
		($hook = ForumFunction::get_hook('aex_uninstall_pre_header_load')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('aex_uninstall_output_start')) ? eval($hook) : null;
...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo $base_url ?>/admin/extensions.php?section=manage&amp;uninstall=<?php echo $id ?>">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token($base_url.'/admin/extensions.php?section=manage&amp;uninstall='.$id) ?>" />
			</div>
			<div class="ct-group data-group">
				<div class="ct-set data-set set1">
					<div class="ct-box data-box">
						<h3 class="ct-legend hn"><span><?php echo ForumFunction::forum_htmlencode($ext_data['title']) ?></span></h3>
						<p><?php echo ((strpos($id, 'hotfix_') !== 0) ? sprintf($lang_admin_ext['Version'], $ext_data['version']) : $lang_admin_ext['Hotfix']) ?></p>
						<p><?php printf($lang_admin_ext['Extension by'], ForumFunction::forum_htmlencode($ext_data['author'])) ?></p>
						<p><?php echo ForumFunction::forum_htmlencode($ext_data['description']) ?></p>
					</div>
				</div>
			</div>
<?php if ($ext_data['uninstall_note'] != ''): ?>			<div class="ct-box warn-box">
				...
				<p><?php echo ForumFunction::forum_htmlencode($ext_data['uninstall_note']) ?></p>
			</div>
<?php endif; ?>
...
<?php
		...
		($hook = ForumFunction::get_hook('aex_uninstall_end')) ? eval($hook) : null;
...
else if (isset($_GET['flip']))
{
	...
	if (!isset($_POST['csrf_token']) && (!isset($_GET['csrf_token']) || $_GET['csrf_token'] !== ForumFunction::generate_form_token('flip'.$id)))
		ForumFunction::csrf_confirm_form();

	($hook = ForumFunction::get_hook('aex_flip_selected')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('aex_flip_qr_get_disabled_status')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	if ($disable)
	{
		...
		($hook = ForumFunction::get_hook('aex_flip_qr_get_disable_dependencies')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		$dependency = $forum_db->fetch_assoc($result);

		if (!is_null($dependency) && $dependency !== false)
		{
			ForumFunction::message(sprintf($lang_admin_ext['Disable dependency'], $dependency['id']));
		}
	}
	else
	{
		...
		($hook = ForumFunction::get_hook('aex_flip_qr_get_enable_dependencies')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		($hook = ForumFunction::get_hook('aex_flip_qr_check_dependencies')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		foreach ($dependencies as $dependency)
		{
			if (!empty($dependency) && !in_array($dependency, $installed_ext))
				ForumFunction::message(sprintf($lang_admin_ext['Disabled dependency'], $dependency));
		}
	}
	...
	($hook = ForumFunction::get_hook('aex_flip_qr_update_disabled_status')) ? eval($hook) : null;
	$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	($hook = ForumFunction::get_hook('aex_flip_pre_redirect')) ? eval($hook) : null;

	if ($section == 'hotfixes')
		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_extensions_hotfixes']), ($disable ? $lang_admin_ext['Hotfix disabled'] : $lang_admin_ext['Hotfix enabled']));
	else
		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_extensions_manage']), ($disable ? $lang_admin_ext['Extension disabled'] : $lang_admin_ext['Extension enabled']));
}

($hook = ForumFunction::get_hook('aex_new_action')) ? eval($hook) : null;
...
($hook = ForumFunction::get_hook('aex_qr_get_all_extensions')) ? eval($hook) : null;
$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
...
if ($section == 'hotfixes')
{
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_common['Forum administration'], ForumFunction::forum_link($forum_url['admin_index'])),
		array($lang_admin_common['Extensions'], ForumFunction::forum_link($forum_url['admin_extensions_manage'])),
		array($lang_admin_common['Manage hotfixes'], ForumFunction::forum_link($forum_url['admin_extensions_hotfixes']))
	);

	($hook = ForumFunction::get_hook('aex_section_hotfixes_pre_header_load')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('aex_section_hotfixes_output_start')) ? eval($hook) : null;
...
<?php
	...
	($hook = ForumFunction::get_hook('aex_section_hotfixes_pre_display_installed_ext_list')) ? eval($hook) : null;
...
<?php
	...
	foreach ($inst_exts as $id => $ext)
	{
		...
		$forum_page['ext_actions'] = array(
			'flip'		=> '<span class="first-item"><a href="'.$base_url.'/admin/extensions.php?section=hotfixes&amp;flip='.$id.'&amp;csrf_token='.ForumFunction::generate_form_token('flip'.$id).'">'.($ext['disabled'] != '1' ? $lang_admin_ext['Disable'] : $lang_admin_ext['Enable']).'</a></span>',
			'uninstall'	=> '<span><a href="'.$base_url.'/admin/extensions.php?section=hotfixese&amp;uninstall='.$id.'">'.$lang_admin_ext['Uninstall'].'</a></span>'
		);

		($hook = ForumFunction::get_hook('aex_section_hotfixes_pre_ext_actions')) ? eval($hook) : null;

?>
		<div class="ct-box info-box hotfix <?php echo $ext['disabled'] == '1' ? 'disabled' : 'enabled' ?>">
			<h3 class="ct-legend hn"><span><?php echo ForumFunction::forum_htmlencode($ext['title']) ?><?php if ($ext['disabled'] == '1') echo ' ( <span>'.$lang_admin_ext['Extension disabled'].'</span> )' ?></span></h3>
			<ul class="data-list">
				<li><span><?php echo ((strpos($id, 'hotfix_') !== 0) ? sprintf($lang_admin_ext['Version'], $ext['version']) : $lang_admin_ext['Hotfix']) ?></span></li>
				<li><span><?php printf($lang_admin_ext['Extension by'], ForumFunction::forum_htmlencode($ext['author'])) ?></span></li>
				<?php if ($ext['description'] != ''): ?>
					<li><span><?php echo ForumFunction::forum_htmlencode($ext['description']) ?></span></li>
				<?php endif; ?>
			</ul>
...
<?php
	...
	($hook = ForumFunction::get_hook('aex_section_hotfixes_end')) ? eval($hook) : null;
...
else
{
	if ($forum_config['o_check_for_versions'] == 1)
	{
		...
		($hook = ForumFunction::get_hook('aex_add_extensions_repository')) ? eval($hook) : null;

		$repository_url_by_extension = array();
		foreach (array_keys($inst_exts) as $id)
			($hook = ForumFunction::get_hook('aex_add_repository_for_'.$id)) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('aex_before_update_checking')) ? eval($hook) : null;
		...
		if ($update_new_versions_cache)
		{
			...
			Cache::generate_ext_versions_cache($inst_exts, $repository_urls, $repository_url_by_extension);
			include FORUM_CACHE_DIR.'cache_ext_version_notifications.php';
		}
	}
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_common['Forum administration'], ForumFunction::forum_link($forum_url['admin_index'])),
		array($lang_admin_common['Extensions'], ForumFunction::forum_link($forum_url['admin_extensions_manage'])),
		array($lang_admin_common['Manage extensions'], ForumFunction::forum_link($forum_url['admin_extensions_manage']))
	);

	($hook = ForumFunction::get_hook('aex_section_manage_pre_header_load')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('aex_section_install_output_start')) ? eval($hook) : null;
...
<?php
	...
	while (($entry = $d->read()) !== false)
	{
		if ($entry{0} != '.' && is_dir(FORUM_ROOT.'extensions/'.$entry))
		{
			...
			{
				$forum_page['ext_error'][] = '<div class="ext-error databox db'.++$forum_page['item_num'].'">'."\n\t\t\t\t".'<h3 class="legend"><span>'.sprintf($lang_admin_ext['Extension loading error'], ForumFunction::forum_htmlencode($entry)).'</span></h3>'."\n\t\t\t\t".'<p>'.$lang_admin_ext['Illegal ID'].'</p>'."\n\t\t\t".'</div>';
				++$num_failed;
				continue;
			}
			...
			else
			{
				...
				{
					$forum_page['ext_item'][] = '<div class="ct-box info-box extension available">'."\n\t\t\t".'<h3 class="ct-legend hn">'.ForumFunction::forum_htmlencode($ext_data['extension']['title']).' <em>'.$ext_data['extension']['version'].'</em></h3>'."\n\t\t\t".'<ul class="data-list">'."\n\t\t\t\t".'<li><span>'.sprintf($lang_admin_ext['Extension by'], ForumFunction::forum_htmlencode($ext_data['extension']['author'])).'</span></li>'.(($ext_data['extension']['description'] != '') ? "\n\t\t\t\t".'<li><span>'.ForumFunction::forum_htmlencode($ext_data['extension']['description']).'</span></li>' : '')."\n\t\t\t".'</ul>'."\n\t\t\t".'<p class="options"><span class="first-item"><a href="'.$base_url.'/admin/extensions.php?install='.urlencode($entry).'">'.(isset($inst_exts[$entry]['version']) ? $lang_admin_ext['Upgrade extension'] : $lang_admin_ext['Install extension']).'</a></span></p>'."\n\t\t".'</div>';
					++$num_exts;
				}
			}
		}
	...
	($hook = ForumFunction::get_hook('aex_section_install_pre_display_available_ext_list')) ? eval($hook) : null;
...
<?php
	...
	($hook = ForumFunction::get_hook('aex_section_manage_pre_display_installed_ext_list')) ? eval($hook) : null;
...
<?php
	...
	foreach ($inst_exts as $id => $ext)
	{
		...
		$forum_page['ext_actions'] = array(
			'flip'		=> '<span class="first-item"><a href="'.$base_url.'/admin/extensions.php?section=manage&amp;flip='.$id.'&amp;csrf_token='.ForumFunction::generate_form_token('flip'.$id).'">'.($ext['disabled'] != '1' ? $lang_admin_ext['Disable'] : $lang_admin_ext['Enable']).'</a></span>',
		...
		($hook = ForumFunction::get_hook('aex_section_manage_pre_ext_actions')) ? eval($hook) : null;

		if ($ext['disabled'] == '1')
			$forum_page['ext_item'][] = '<div class="ct-box info-box extension disabled">'."\n\t\t".'<h3 class="ct-legend hn">'.ForumFunction::forum_htmlencode($ext['title']).' <em>'.$ext['version'].'</em> ('.$lang_admin_ext['Extension disabled'].')</h3>'."\n\t\t".'<ul class="data-list">'."\n\t\t\t".'<li><span>'.sprintf($lang_admin_ext['Extension by'], ForumFunction::forum_htmlencode($ext['author'])).'</span></li>'."\n\t\t\t".(($ext['description'] != '') ? '<li><span>'.ForumFunction::forum_htmlencode($ext['description']).'</span></li>' : '')."\n\t\t\t".'</ul>'."\n\t\t".'<p class="options">'.implode(' ', $forum_page['ext_actions']).'</p>'."\n\t".'</div>';
		else
			$forum_page['ext_item'][] = '<div class="ct-box info-box extension enabled">'."\n\t\t".'<h3 class="ct-legend hn">'.ForumFunction::forum_htmlencode($ext['title']).' <em>'.$ext['version'].'</em></h3>'."\n\t\t".'<ul class="data-list">'."\n\t\t\t".'<li><span>'.sprintf($lang_admin_ext['Extension by'], ForumFunction::forum_htmlencode($ext['author'])).'</span></li>'."\n\t\t\t".(($ext['description'] != '') ? '<li><span>'.ForumFunction::forum_htmlencode($ext['description']).'</span></li>' : '')."\n\t\t".'</ul>'."\n\t\t".'<p class="options">'.implode(' ', $forum_page['ext_actions']).'</p>'."\n\t".'</div>';
...
<?php
	...
	($hook = ForumFunction::get_hook('aex_section_manage_end')) ? eval($hook) : null;
