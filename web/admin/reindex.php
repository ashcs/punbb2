...
($hook = ForumFunction::get_hook('ari_start')) ? eval($hook) : null;
...
if (isset($_GET['i_per_page']) && isset($_GET['i_start_at']))
{
	...
	if (!isset($_POST['csrf_token']) && (!isset($_GET['csrf_token']) || $_GET['csrf_token'] !== ForumFunction::generate_form_token('reindex'.$forum_user['id'])))
		ForumFunction::csrf_confirm_form();

	($hook = ForumFunction::get_hook('ari_cycle_start')) ? eval($hook) : null;
	...
	if (isset($_GET['i_empty_index']))
	{
		...
		($hook = ForumFunction::get_hook('ari_cycle_qr_empty_search_matches')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('ari_cycle_qr_empty_search_words')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		switch ($db_type)
		{
			...
			case 'mysqli':
			case 'mysqli_innodb':
				$result = $forum_db->query('ALTER TABLE '.$forum_db->prefix.'search_words auto_increment=1') or ForumFunction::error(__FILE__, __LINE__);
				break;

			case 'pgsql';
				$result = $forum_db->query('SELECT setval(\''.$forum_db->prefix.'search_words_id_seq\', 1, false)') or ForumFunction::error(__FILE__, __LINE__);
		}
	}
...
<title><?php echo ForumFunction::generate_crumbs(true) ?></title>
<style type="text/css">
body {
...
<?php
	...
	($hook = ForumFunction::get_hook('ari_cycle_qr_fetch_posts')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('ari_cycle_qr_find_next_post')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	{
		$query_str = '?i_per_page='.$per_page.'&i_start_at='.$next_posts_to_proced.'&csrf_token='.ForumFunction::generate_form_token('reindex'.$forum_user['id']);
	}

	($hook = ForumFunction::get_hook('ari_cycle_end')) ? eval($hook) : null;

	$forum_db->end_transaction();
	$forum_db->close();

	exit('<script type="text/javascript">window.location="'.ForumFunction::forum_link($forum_url['admin_reindex']).$query_str.'"</script><br />'.$lang_admin_reindex['Javascript redirect'].' <a href="'.ForumFunction::forum_link($forum_url['admin_reindex']).$query_str.'">'.$lang_admin_reindex['Click to continue'].'</a>.');
}
...
($hook = ForumFunction::get_hook('ari_qr_find_lowest_post_id')) ? eval($hook) : null;
$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
...
$forum_page['crumbs'] = array(
	...
	array($lang_admin_common['Forum administration'], ForumFunction::forum_link($forum_url['admin_index'])),
	array($lang_admin_common['Management'], ForumFunction::forum_link($forum_url['admin_reports'])),
	array($lang_admin_common['Rebuild index'], ForumFunction::forum_link($forum_url['admin_reindex']))
);

($hook = ForumFunction::get_hook('ari_pre_header_load')) ? eval($hook) : null;
...
($hook = ForumFunction::get_hook('ari_main_output_start')) ? eval($hook) : null;
...
?>
	...
	<div class="main-content main-frm">
		...
		<form class="frm-form" method="get" accept-charset="utf-8" action="<?php echo ForumFunction::forum_link($forum_url['admin_reindex']) ?>">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token('reindex'.$forum_user['id']) ?>" />
			</div>
<?php ($hook = ForumFunction::get_hook('ari_pre_rebuild_fieldset')) ? eval($hook) : null; ?>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><span><?php echo $lang_admin_reindex['Rebuild index legend'] ?></span></legend>
<?php ($hook = ForumFunction::get_hook('ari_pre_rebuild_per_page')) ? eval($hook) : null; ?>
...
<?php ($hook = ForumFunction::get_hook('ari_pre_rebuild_start_post')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
...
<?php ($hook = ForumFunction::get_hook('ari_pre_rebuild_empty_index')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
...
<?php ($hook = ForumFunction::get_hook('ari_pre_rebuild_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php ($hook = ForumFunction::get_hook('ari_rebuild_fieldset_end')) ? eval($hook) : null; ?>
...
($hook = ForumFunction::get_hook('ari_end')) ? eval($hook) : null;
