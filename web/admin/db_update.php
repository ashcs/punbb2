...
$autoloader->add('', dirname(__DIR__) . '/../classes');
...
if (version_compare($cur_version, '1.2', '<'))
 ForumFunction::error('Version mismatch. The database \''.$db_name.'\' doesn\'t seem to be running a PunBB database schema supported by this update script.', __FILE__, __LINE__);
...
if (in_array($db_type, array('mysql', 'mysqli', 'mysql_innodb', 'mysqli_innodb')))
{
	...
	if (version_compare($mysql_info['version'], MIN_MYSQL_VERSION, '<'))
	 ForumFunction::error('You are running MySQL version '.$mysql_version.'. PunBB '.UPDATE_TO.' requires at least MySQL '.MIN_MYSQL_VERSION.' to run properly. You must upgrade your MySQL installation before you can continue.');
}
...
if (isset($forum_config['o_database_revision']) && $forum_config['o_database_revision'] >= UPDATE_TO_DB_REVISION && version_compare($forum_config['o_cur_version'], UPDATE_TO, '>='))
 ForumFunction::error('Your database is already as up-to-date as this script can make it.');
...
function convert_table_utf8($table)
{
	...
	$forum_db->query('ALTER TABLE `'.$table.'` CHARACTER SET utf8') or ForumFunction::error(__FILE__, __LINE__);

	// Find out which columns need converting and build SQL statements
	$result = $forum_db->query('SHOW FULL COLUMNS FROM `'.$table.'`') or ForumFunction::error(__FILE__, __LINE__);
...
$current_url = ForumFunction::get_current_url();
if (strpos($cur_version, '1.2') === 0 && $db_seems_utf8 && !isset($_GET['force']))
{
...
<?php
	...
	case 'start':
		...
		if (in_array($db_type, array('mysql', 'mysqli', 'mysql_innodb', 'mysqli_innodb')))
		{
			$result = $forum_db->query('SHOW FULL COLUMNS FROM '.$forum_db->prefix.'search_words') or ForumFunction::error(__FILE__, __LINE__);
			while ($cur_column = $forum_db->fetch_assoc($result))
			{
	...
	case 'finish':
		...
		while ($row = $forum_db->fetch_row($result))
			ForumFunction::sync_forum($row[0]);
			...
			<fieldset class="frm-group group1">
				...
				<div class="txt-set set1">
					<div class="txt-box textarea">
						...
						<div class="txt-input"><span class="frm-input"><textarea id="fld1" readonly="readonly" cols="80" rows="20"><?php echo ForumFunction::forum_htmlencode($new_config) ?></textarea></span></div>
					</div>
				</div>
			</fieldset>
		</form>
