...
($hook = ForumFunction::get_hook('ain_start')) ? eval($hook) : null;
...
{
	($hook = ForumFunction::get_hook('ain_phpinfo_selected')) ? eval($hook) : null;

	// Is phpinfo() a disabled function?
	if (strpos(strtolower((string)@ini_get('disable_functions')), 'phpinfo') !== false)
		ForumFunction::message($lang_admin_index['phpinfo disabled']);
...
if ($forum_user['g_id'] == FORUM_ADMIN)
{
	...
	else
	{
		...
		($hook = ForumFunction::get_hook('ain_update_check_qr_get_hotfixes')) ? eval($hook) : null;
...
{
	array_walk($load_averages, create_function('&$v', '$v = ForumFunction::forum_number_format(round($v, 2), 2);'));
...
else if (@/**/is_readable('/proc/loadavg'))
{
	...
	$server_load = isset($load_averages[2]) ? ForumFunction::forum_number_format(round($load_averages[0], 2), 2).' '.ForumFunction::forum_number_format(round($load_averages[1], 2), 2).' '.ForumFunction::forum_number_format(round($load_averages[2], 2), 2) : 'Not available';
}
else if (!in_array(PHP_OS, array('WINNT', 'WIN32')) && preg_match('/averages?: ([0-9\.]+),[\s]+([0-9\.]+),[\s]+([0-9\.]+)/i', @exec('uptime'), $load_averages))
	$server_load = ForumFunction::forum_number_format(round($load_averages[1], 2), 2).' '.ForumFunction::forum_number_format(round($load_averages[2], 2), 2).' '.ForumFunction::forum_number_format(round($load_averages[3], 2), 2);
else
...
($hook = ForumFunction::get_hook('ain_qr_get_users_online')) ? eval($hook) : null;
$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
...
if (in_array($db_type, array('mysql', 'mysqli', 'mysql_innodb', 'mysqli_innodb')))
{
	...
	$result = $forum_db->query('SHOW TABLE STATUS FROM `'.$db_name.'` LIKE \''.$db_prefix.'%\'') or ForumFunction::error(__FILE__, __LINE__);
	...
	if ($total_size > 1024)
		$total_size = ForumFunction::forum_number_format($total_size / 1024, 2).' MB';
	else
		$total_size = ForumFunction::forum_number_format($total_size, 2).' KB';
}
...
if ($forum_user['g_id'] == FORUM_ADMIN)
	$forum_page['crumbs'][] = array($lang_admin_common['Start'], ForumFunction::forum_link($forum_url['admin_index']));
$forum_page['crumbs'][] = array($lang_admin_common['Information'], ForumFunction::forum_link($forum_url['admin_index']));

($hook = ForumFunction::get_hook('ain_pre_header_load')) ? eval($hook) : null;
...
($hook = ForumFunction::get_hook('ain_main_output_start')) ? eval($hook) : null;
...
<?php endif; ?>
		<div class="ct-group">
<?php ($hook = ForumFunction::get_hook('ain_pre_version')) ? eval($hook) : null; ?>
...
<?php ($hook = ForumFunction::get_hook('ain_pre_community')) ? eval($hook) : null; ?>
			<div class="ct-set group-item<?php echo ++$forum_page['item_count'] ?>">
				<div class="ct-box">
...
<?php ($hook = ForumFunction::get_hook('ain_pre_server_load')) ? eval($hook) : null; ?>
			<div class="ct-set group-item<?php echo ++$forum_page['item_count'] ?>">
				<div class="ct-box">
...
<?php ($hook = ForumFunction::get_hook('ain_pre_environment')) ? eval($hook) : null; if ($forum_user['g_id'] == FORUM_ADMIN): ?>
			<div class="ct-set group-item<?php echo ++$forum_page['item_count'] ?>">
				<div class="ct-box">
					...
					<ul class="data-list">
						...
						<li><span>PHP: <?php echo PHP_VERSION ?> - <a href="<?php echo ForumFunction::forum_link($forum_url['admin_index']) ?>?action=phpinfo"><?php echo $lang_admin_index['Show info'] ?></a></span></li>
						<li><span><?php echo $lang_admin_index['Accelerator'] ?>: <?php echo $php_accelerator ?></span></li>
					</ul>
				</div>
			</div>
<?php ($hook = ForumFunction::get_hook('ain_pre_database')) ? eval($hook) : null; ?>
			<div class="ct-set group-item<?php echo ++$forum_page['item_count'] ?>">
				<div class="ct-box">
...
<?php if (isset($total_records) && isset($total_size)): ?>
						<li><span><?php echo $lang_admin_index['Rows'] ?>: <?php echo ForumFunction::forum_number_format($total_records) ?></span></li>
						<li><span><?php echo $lang_admin_index['Size'] ?>: <?php echo $total_size ?></span></li>
<?php endif; ?>
					</ul>
				</div>
			</div>
<?php endif; ($hook = ForumFunction::get_hook('ain_items_end')) ? eval($hook) : null; ?>
		</div>
	</div>
<?php

($hook = ForumFunction::get_hook('ain_end')) ? eval($hook) : null;
