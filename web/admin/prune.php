...
($hook = ForumFunction::get_hook('apr_start')) ? eval($hook) : null;
...
if (isset($_GET['action']) || isset($_POST['prune']) || isset($_POST['prune_comply']))
{
	if (isset($_POST['prune_comply']))
	{
		...
		($hook = ForumFunction::get_hook('apr_prune_comply_form_submitted')) ? eval($hook) : null;
		...
		if ($prune_from == 'all')
		{
			...
			($hook = ForumFunction::get_hook('apr_prune_comply_qr_get_all_forums')) ? eval($hook) : null;
			...
			while ($cur_forum = $forum_db->fetch_assoc($result)) {
				...
				ForumFunction::sync_forum($cur_forum['id']);
			}
		}
		else
		{
			...
			ForumFunction::sync_forum($prune_from);
		}
		...
		($hook = ForumFunction::get_hook('apr_prune_pre_redirect')) ? eval($hook) : null;

		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_prune']), $lang_admin_prune['Prune done']);
	}


	$prune_days = intval($_POST['req_prune_days']);
	if ($prune_days < 0)
		ForumFunction::message($lang_admin_prune['Days to prune message']);
	...
	if ($prune_from != 'all')
	{
		...
		($hook = ForumFunction::get_hook('apr_prune_comply_qr_get_forum_name')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		$forum = ForumFunction::forum_htmlencode($forum_db->result($result));
	}
	else
		$forum = 'all forums';
	...
	($hook = ForumFunction::get_hook('apr_prune_comply_qr_get_topic_count')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	$num_topics = $forum_db->result($result);

	if (!$num_topics)
		ForumFunction::message($lang_admin_prune['No days old message']);
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_prune['Prune topics'], ForumFunction::forum_link($forum_url['admin_prune'])),
		$lang_admin_prune['Confirm prune heading']
	);

	($hook = ForumFunction::get_hook('apr_prune_comply_pre_header_load')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('apr_prune_comply_output_start')) ? eval($hook) : null;
...
?>
	...
	<div class="main-content main-frm">
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo ForumFunction::forum_link($forum_url['admin_prune']) ?>?action=foo">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo ForumFunction::generate_form_token(ForumFunction::forum_link($forum_url['admin_prune']).'?action=foo') ?>" />
...
<?php ($hook = ForumFunction::get_hook('apr_prune_comply_pre_buttons')) ? eval($hook) : null; ?>
...
<?php
	...
	($hook = ForumFunction::get_hook('apr_prune_comply_end')) ? eval($hook) : null;
...
else
{
	...
	$forum_page['crumbs'] = array(
		...
		array($lang_admin_common['Forum administration'], ForumFunction::forum_link($forum_url['admin_index'])),
		array($lang_admin_common['Management'], ForumFunction::forum_link($forum_url['admin_reports'])),
		array($lang_admin_common['Prune topics'], ForumFunction::forum_link($forum_url['admin_prune']))
	);

	($hook = ForumFunction::get_hook('apr_pre_header_load')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('apr_main_output_start')) ? eval($hook) : null;
...
<?php ($hook = ForumFunction::get_hook('apr_pre_prune_fieldset')) ? eval($hook) : null; ?>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><span><?php echo $lang_admin_prune['Prune legend'] ?></span></legend>
<?php ($hook = ForumFunction::get_hook('apr_pre_prune_from')) ? eval($hook) : null; ?>
...
<?php
	...
	($hook = ForumFunction::get_hook('apr_qr_get_forum_list')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);

	$cur_category = 0;
	while ($forum = $forum_db->fetch_assoc($result))
	{
		($hook = ForumFunction::get_hook('apr_pre_prune_forum_loop_start')) ? eval($hook) : null;
		...
		if ($forum['cid'] != $cur_category)	// Are we still in the same category?
		{
			...
			echo "\t\t\t\t\t\t\t\t".'<optgroup label="'.ForumFunction::forum_htmlencode($forum['cat_name']).'">'."\n";
			$cur_category = $forum['cid'];
		}

		echo "\t\t\t\t\t\t\t\t\t".'<option value="'.$forum['fid'].'">'.ForumFunction::forum_htmlencode($forum['forum_name']).'</option>'."\n";
		
		($hook = ForumFunction::get_hook('apr_pre_prune_forum_loop_end')) ? eval($hook) : null;
	}
...
<?php ($hook = ForumFunction::get_hook('apr_pre_prune_days')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
...
<?php ($hook = ForumFunction::get_hook('apr_pre_prune_sticky')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
...
<?php ($hook = ForumFunction::get_hook('apr_pre_prune_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php ($hook = ForumFunction::get_hook('apr_prune_fieldset_end')) ? eval($hook) : null; ?>
			<div class="frm-buttons">
...
<?php
	...
	($hook = ForumFunction::get_hook('apr_end')) ? eval($hook) : null;
