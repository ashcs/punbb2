...
($hook = ForumFunction::get_hook('acg_start')) ? eval($hook) : null;
...
{
	$new_cat_name = ForumFunction::forum_trim($_POST['new_cat_name']);
	if ($new_cat_name == '')
		ForumFunction::message($lang_admin_categories['Must name category']);

	$new_cat_pos = intval($_POST['position']);

	($hook = ForumFunction::get_hook('acg_add_cat_form_submitted')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('acg_add_cat_qr_add_category')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('acg_add_cat_pre_redirect')) ? eval($hook) : null;

	ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_categories']), $lang_admin_categories['Category added']);
}
...
else if (isset($_POST['del_cat']) || isset($_POST['del_cat_comply']))
{
	...
	if (isset($_POST['del_cat_cancel']))
		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_categories']), $lang_admin_common['Cancel redirect']);

	($hook = ForumFunction::get_hook('acg_del_cat_form_submitted')) ? eval($hook) : null;
	...
	if (isset($_POST['del_cat_comply']))	// Delete a category with all forums and posts
	{
		...
		($hook = ForumFunction::get_hook('acg_del_cat_qr_get_forums_to_delete')) ? eval($hook) : null;
		...
		if (!empty($forum_ids))
		{
			foreach ($forum_ids as $cur_forum)
			{
				...
				($hook = ForumFunction::get_hook('acg_del_cat_qr_delete_forum')) ? eval($hook) : null;
				$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
				...
				($hook = ForumFunction::get_hook('acg_del_cat_qr_delete_forum_subscriptions')) ? eval($hook) : null;
				$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
			}
		}
		...
		($hook = ForumFunction::get_hook('acg_del_cat_qr_delete_category')) ? eval($hook) : null;
		$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		($hook = ForumFunction::get_hook('acg_del_cat_pre_redirect')) ? eval($hook) : null;

		ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_categories']), $lang_admin_categories['Category deleted']);
	}
	else	// If the user hasn't comfirmed the delete
	{
		...
		($hook = ForumFunction::get_hook('acg_del_cat_qr_get_category_name')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		// Setup the form
		$forum_page['form_action'] = ForumFunction::forum_link($forum_url['admin_categories']);
		...
		$forum_page['crumbs'] = array(
			...
			array($lang_admin_common['Categories'], ForumFunction::forum_link($forum_url['admin_categories'])),
			$lang_admin_categories['Delete category']
		);

		($hook = ForumFunction::get_hook('acg_del_cat_pre_header_load')) ? eval($hook) : null;
		...
		($hook = ForumFunction::get_hook('acg_del_cat_output_start')) ? eval($hook) : null;

?>
	<div class="main-subhead">
		<h2 class="hn"><span><?php printf($lang_admin_categories['Confirm delete cat'], ForumFunction::forum_htmlencode($cat_name)) ?></span></h2>
	</div>
...
<?php
		...
		($hook = ForumFunction::get_hook('acg_del_cat_end')) ? eval($hook) : null;
...
else if (isset($_POST['update']))	// Change position and name of the categories
{
	...
	($hook = ForumFunction::get_hook('acg_update_cats_form_submitted')) ? eval($hook) : null;
	...
	($hook = ForumFunction::get_hook('acg_update_cats_qr_get_categories')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	while ($cur_cat = $forum_db->fetch_assoc($result))
	{
		...
		if (isset($cat_name[$cur_cat['id']]) && isset($cat_order[$cur_cat['id']]))
		{
			...
			if ($cat_order[$cur_cat['id']] < 0)
				ForumFunction::message($lang_admin_categories['Must be integer']);
			...
			if ($cur_cat['cat_name'] != $cat_name[$cur_cat['id']] || $cur_cat['disp_position'] != $cat_order[$cur_cat['id']])
			{
				...
				($hook = ForumFunction::get_hook('acg_update_cats_qr_update_category')) ? eval($hook) : null;
				$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
			}
		}
	}
	...
	($hook = ForumFunction::get_hook('acg_update_cats_pre_redirect')) ? eval($hook) : null;

	ForumFunction::redirect(ForumFunction::forum_link($forum_url['admin_categories']), $lang_admin_categories['Categories updated']);
}
...
($hook = ForumFunction::get_hook('acg_qr_get_categories')) ? eval($hook) : null;
$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
...
$forum_page['form_action'] = ForumFunction::forum_link($forum_url['admin_categories']).'?action=foo';
...
($hook = ForumFunction::get_hook('acg_pre_header_load')) ? eval($hook) : null;
...
($hook = ForumFunction::get_hook('acg_main_output_start')) ? eval($hook) : null;
...
<?php ($hook = ForumFunction::get_hook('acg_pre_add_cat_fieldset')) ? eval($hook) : null; ?>
			<div class="ct-box">
				<p><?php printf($lang_admin_categories['Add category info'], '<a href="'.ForumFunction::forum_link($forum_url['admin_forums']).'">'.$lang_admin_categories['Add category info link text'].'</a>') ?></p>
			</div>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><span><?php echo $lang_admin_categories['Add category legend'] ?></span></legend>
<?php ($hook = ForumFunction::get_hook('acg_pre_new_category_name')) ? eval($hook) : null; ?>
...
<?php ($hook = ForumFunction::get_hook('acg_pre_new_category_position')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text">
...
<?php ($hook = ForumFunction::get_hook('acg_pre_add_cat_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php ($hook = ForumFunction::get_hook('acg_add_cat_fieldset_end')) ? eval($hook) : null; ?>
			<div class="frm-buttons">
...
($hook = ForumFunction::get_hook('acg_post_add_cat_form')) ? eval($hook) : null;
...
<?php ($hook = ForumFunction::get_hook('acg_pre_del_cat_fieldset')) ? eval($hook) : null; ?>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><strong><?php echo $lang_admin_categories['Delete category'] ?></strong></legend>
<?php ($hook = ForumFunction::get_hook('acg_pre_del_category_select')) ? eval($hook) : null; ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
...
<?php
	...
	{
		echo "\t\t\t\t\t\t\t".'<option value="'.$cur_category['id'].'">'.ForumFunction::forum_htmlencode($cur_category['cat_name']).'</option>'."\n";
	}
...
<?php ($hook = ForumFunction::get_hook('acg_pre_del_cat_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php ($hook = ForumFunction::get_hook('acg_del_cat_fieldset_end')) ? eval($hook) : null; ?>
			<div class="frm-buttons">
...
($hook = ForumFunction::get_hook('acg_post_del_cat_form')) ? eval($hook) : null;
...
<?php
	...
	($hook = ForumFunction::get_hook('acg_edit_cat_fieldsets_start')) ? eval($hook) : null;
	foreach ($cat_list as $cur_category)
	{
		$forum_page['item_count'] = 0;
		($hook = ForumFunction::get_hook('acg_pre_edit_cur_cat_fieldset')) ? eval($hook) : null;

?>
			<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
				<legend class="group-legend"><span><?php echo $lang_admin_categories['Edit category legend'] ?></span></legend>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
<?php ($hook = ForumFunction::get_hook('acg_pre_edit_cat_name')) ? eval($hook) : null; ?>
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_admin_categories['Category name label'] ?></span></label><br />
						<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="cat_name[<?php echo $cur_category['id'] ?>]" value="<?php echo ForumFunction::forum_htmlencode($cur_category['cat_name']) ?>" size="35" maxlength="80" required /></span>
					</div>
<?php ($hook = ForumFunction::get_hook('acg_pre_edit_cat_position')) ? eval($hook) : null; ?>
					<div class="sf-box text">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_admin_categories['Position label'] ?></span></label><br />
...
<?php ($hook = ForumFunction::get_hook('acg_pre_edit_cur_cat_fieldset_end')) ? eval($hook) : null; ?>
			</fieldset>
<?php

		($hook = ForumFunction::get_hook('acg_edit_cur_cat_fieldset_end')) ? eval($hook) : null;
	}

	($hook = ForumFunction::get_hook('acg_edit_cat_fieldsets_end')) ? eval($hook) : null;
...
<?php
	...
	($hook = ForumFunction::get_hook('acg_post_edit_cat_form')) ? eval($hook) : null;
}

($hook = ForumFunction::get_hook('acg_end')) ? eval($hook) : null;
