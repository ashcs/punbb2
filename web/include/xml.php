...
function xml_to_array($raw_xml)
{
	...
	foreach ($vals as $xml_elem)
	{
		...
		if (isset($xml_elem['value']) && ForumFunction::forum_trim($xml_elem['value']) != '' && !array_key_exists('attributes', $xml_elem))
		{
			if ($x_type == 'open')
...
function validate_manifest($xml_array, $folder_name)
{
	...
	$return = ($hook = ForumFunction::get_hook('xm_fn_validate_manifest_start')) ? eval($hook) : null;
	if ($return !== null)
		return;
	...
	else
	{
		...
		if (isset($ext['minversion']) && version_compare(ForumFunction::clean_version($forum_config['o_cur_version']), ForumFunction::clean_version($ext['minversion']), '<'))
	...
	($hook = ForumFunction::get_hook('xm_fn_validate_manifest_end')) ? eval($hook) : null;
