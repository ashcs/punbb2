...
{
	$return = ($hook = ForumFunction::get_hook('em_fn_is_valid_email_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function is_banned_email($email)
{
	...
	$return = ($hook = ForumFunction::get_hook('em_fn_is_banned_email_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function forum_mail($to, $subject, $message, $reply_to_email = '', $reply_to_name = '')
{
	...
	($hook = ForumFunction::get_hook('em_fn_forum_mail_start')) ? eval($hook) : null;

	// Do a little spring cleaning
	$to = ForumFunction::forum_trim(preg_replace('#[\n\r]+#s', '', $to));
	$subject = ForumFunction::forum_trim(preg_replace('#[\n\r]+#s', '', $subject));
	$from_email = ForumFunction::forum_trim(preg_replace('#[\n\r:]+#s', '', $from_email));
	$from_name = ForumFunction::forum_trim(preg_replace('#[\n\r:]+#s', '', str_replace('"', '', $from_name)));
	$reply_to_email = ForumFunction::forum_trim(preg_replace('#[\n\r:]+#s', '', $reply_to_email));
	$reply_to_name = ForumFunction::forum_trim(preg_replace('#[\n\r:]+#s', '', str_replace('"', '', $reply_to_name)));
	...
	$message = str_replace(array("\n", "\0"), array("\r\n", ''), ForumFunction::forum_linebreaks($message));

	($hook = ForumFunction::get_hook('em_fn_forum_mail_pre_send')) ? eval($hook) : null;
...
function server_parse($socket, $expected_response)
{
	...
	while (substr($server_response, 3, 1) != ' ')
	{
		if (!($server_response = fgets($socket, 256)))
		 ForumFunction::error('Couldn\'t get mail server response codes.<br />Please contact the forum administrator.', __FILE__, __LINE__);
	}

	if (!(substr($server_response, 0, 3) == $expected_response))
	 ForumFunction::error('Unable to send e-mail.<br />Please contact the forum administrator with the following error message reported by the SMTP server: "'.$server_response.'"', __FILE__, __LINE__);
}
...
function smtp_mail($to, $subject, $message, $headers = '')
{
	...
	if (!($socket = fsockopen($smtp_host, $smtp_port, $errno, $errstr, 15)))
	 ForumFunction::error('Could not connect to smtp host "'.$forum_config['o_smtp_host'].'" ('.$errno.') ('.$errstr.').', __FILE__, __LINE__);
