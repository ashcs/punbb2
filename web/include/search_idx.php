...
function split_words($text)
{
	...
	$words = array_filter($words, 'ForumFunction::validate_search_word');

	$return = ($hook = ForumFunction::get_hook('si_fn_split_words_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function update_search_index($mode, $post_id, $message, $subject = null)
{
	...
	$return = ($hook = ForumFunction::get_hook('si_fn_update_search_index_start')) ? eval($hook) : null;
	if ($return !== null)
		return;
	...
	if ($mode == 'edit')
	{
		...
		($hook = ForumFunction::get_hook('si_fn_update_search_index_qr_get_current_words')) ? eval($hook) : null;
	...
	if (!empty($unique_words))
	{
		...
		($hook = ForumFunction::get_hook('si_fn_update_search_index_qr_get_existing_words')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		if (!empty($new_words))
		{
			...
			($hook = ForumFunction::get_hook('si_fn_update_search_index_qr_insert_words')) ? eval($hook) : null;
	...
	foreach ($words['del'] as $match_in => $wordlist)
	{
		if (!empty($wordlist))
		{
			...
			($hook = ForumFunction::get_hook('si_fn_update_search_index_qr_delete_matches')) ? eval($hook) : null;
			$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		}
	}
	...
	foreach ($words['add'] as $match_in => $wordlist)
	{
		if (!empty($wordlist))
		{
			...
			($hook = ForumFunction::get_hook('si_fn_update_search_index_qr_add_matches')) ? eval($hook) : null;
			$forum_db->query($sql) or ForumFunction::error(__FILE__, __LINE__);
		}
	}
...
function strip_search_index($post_ids)
{
	...
	$return = ($hook = ForumFunction::get_hook('si_fn_strip_search_index_start')) ? eval($hook) : null;
	if ($return !== null)
		return;
	...
	($hook = ForumFunction::get_hook('si_fn_strip_search_index_qr_get_post_words')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	if (!empty($word_ids))
	{
		...
		($hook = ForumFunction::get_hook('si_fn_strip_search_index_qr_get_removable_words')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		...
		if (!empty($word_ids))
		{
			...
			($hook = ForumFunction::get_hook('si_fn_strip_search_index_qr_delete_words')) ? eval($hook) : null;
			$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		}
	}
	...
	($hook = ForumFunction::get_hook('si_fn_strip_search_index_qr_delete_matches')) ? eval($hook) : null;
	$forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);

	($hook = ForumFunction::get_hook('si_fn_strip_search_index_end')) ? eval($hook) : null;
}
