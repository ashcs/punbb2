...
$GLOBALS['smilies'] = array(':)' => 'smile.png', '=)' => 'smile.png', ':|' => 'neutral.png', '=|' => 'neutral.png', ':(' => 'sad.png', '=(' => 'sad.png', ':D' => 'big_smile.png', '=D' => 'big_smile.png', ':o' => 'yikes.png', ':O' => 'yikes.png', ';)' => 'wink.png', ':/' => 'hmm.png', ':P' => 'tongue.png', ':p' => 'tongue.png', ':lol:' => 'lol.png', ':mad:' => 'mad.png', ':rolleyes:' => 'roll.png', ':cool:' => 'cool.png');

($hook = ForumFunction::get_hook('ps_start')) ? eval($hook) : null;
...
function preparse_bbcode($text, &$errors, $is_signature = false)
{
	...
	$return = ($hook = ForumFunction::get_hook('ps_preparse_bbcode_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_preparse_bbcode_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;

	return ForumFunction::forum_trim($text);
}
...
function preparse_tags($text, &$errors, $is_signature = false)
{
	...
	$return = ($hook = ForumFunction::get_hook('ps_preparse_tags_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	foreach ($split_text as $current)
	{
		...
		if (substr($current, 0, 1) != '[' || substr($current, -1, 1) != ']')
		{
			...
			if (in_array($open_tags[$opened_tag], $tags_trim))
				$new_text .= ForumFunction::forum_trim($current);
			else
				$new_text .= $current;
	...
	$return = ($hook = ForumFunction::get_hook('ps_preparse_tags_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function preparse_list_tag($content, $type = '*', &$errors)
{
	...
	{
		if (ForumFunction::forum_trim($item) != '')
			$content .= '[*'."\0".']'.str_replace('[/*]', '', ForumFunction::forum_trim($item)).'[/*'."\0".']'."\n";
	}
...
{
	$return = ($hook = ForumFunction::get_hook('ps_handle_url_tag_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
*/
	...
	$return = ($hook = ForumFunction::get_hook('ps_handle_url_tag_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function handle_img_tag($url, $is_signature = false, $alt = null)
{
	...
	$return = ($hook = ForumFunction::get_hook('ps_handle_img_tag_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	if ($is_signature && $forum_user['show_img_sig'] != '0')
		$img_tag = '<img class="sigimage" src="'.$url.'" alt="'.ForumFunction::forum_htmlencode($alt).'" />';
	else if (!$is_signature && $forum_user['show_img'] != '0')
		$img_tag = '<span class="postimg"><img src="'.$url.'" alt="'.ForumFunction::forum_htmlencode($alt).'" /></span>';

	$return = ($hook = ForumFunction::get_hook('ps_handle_img_tag_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function handle_list_tag($content, $type = '*')
{
	...
	$content = preg_replace('#\s*\[\*\](.*?)\[/\*\]\s*#s', '<li><p>$1</p></li>', ForumFunction::forum_trim($content));
...
function do_bbcode($text, $is_signature = false)
{
	...
	$return = ($hook = ForumFunction::get_hook('ps_do_bbcode_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
$text);
	...
	$return = ($hook = ForumFunction::get_hook('ps_do_bbcode_replace')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_do_bbcode_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function do_smilies($text)
{
	...
	$return = ($hook = ForumFunction::get_hook('ps_do_smilies_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_do_smilies_end')) ? eval($hook) : null;
...
function parse_message($text, $hide_smilies)
{
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_message_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;

	if ($forum_config['o_censoring'] == '1')
		$text = ForumFunction::censor_words($text);

	$return = ($hook = ForumFunction::get_hook('ps_parse_message_post_censor')) ? eval($hook) : null;
	if ($return !== null)
		return $return;

	// Convert applicable characters to HTML entities
	$text = ForumFunction::forum_htmlencode($text);

	$return = ($hook = ForumFunction::get_hook('ps_parse_message_pre_split')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_message_post_split')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_message_bbcode')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_message_pre_merge')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	if (isset($inside))
	{
		...
		for ($i = 0; $i < $num_tokens; ++$i)
		{
			$text .= $outside[$i];
			if (isset($inside[$i]))
				$text .= '</p><div class="codebox"><pre><code>'.ForumFunction::forum_trim($inside[$i], "\n\r").'</code></pre></div><p>';
		}
	}

	$return = ($hook = ForumFunction::get_hook('ps_parse_message_post_merge')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_message_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
...
function parse_signature($text)
{
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_signature_start')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_signature_post_censor')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_signature_pre_bbcode')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_signature_post_bbcode')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
	...
	$return = ($hook = ForumFunction::get_hook('ps_parse_signature_end')) ? eval($hook) : null;
	if ($return !== null)
		return $return;
