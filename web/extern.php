...
($hook = ForumFunction::get_hook('ex_start')) ? eval($hook) : null;
...
if ($forum_user['is_guest'] && isset($_SERVER['PHP_AUTH_USER']))
	ForumFunction::authenticate_user($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']);
...
function output_rss($feed)
{
	...
	echo "\t".'<channel>'."\n";
	echo "\t\t".'<title><![CDATA['.ForumFunction::escape_cdata($feed['title']).']]></title>'."\n";
	echo "\t\t".'<link>'.$feed['link'].'</link>'."\n";
	echo "\t\t".'<atom:link href="'.ForumFunction::forum_htmlencode(ForumFunction::get_current_url()).'" rel="self" type="application/rss+xml" />'."\n";
	echo "\t\t".'<description><![CDATA['.ForumFunction::escape_cdata($feed['description']).']]></description>'."\n";
	...
	($hook = ForumFunction::get_hook('ex_add_new_rss_info')) ? eval($hook) : null;

	foreach ($feed['items'] as $item)
	{
		echo "\t\t".'<item>'."\n";
		echo "\t\t\t".'<title><![CDATA['.ForumFunction::escape_cdata($item['title']).']]></title>'."\n";
		echo "\t\t\t".'<link>'.$item['link'].'</link>'."\n";
		echo "\t\t\t".'<description><![CDATA['.ForumFunction::escape_cdata($item['description']).']]></description>'."\n";
		echo "\t\t\t".'<author><![CDATA['.(isset($item['author']['email']) ? ForumFunction::escape_cdata($item['author']['email']) : 'null@example.com').' ('.ForumFunction::escape_cdata($item['author']['name']).')]]></author>'."\n";
		echo "\t\t\t".'<pubDate>'.gmdate('r', $item['pubdate']).'</pubDate>'."\n";
		echo "\t\t\t".'<guid>'.$item['link'].'</guid>'."\n";

		($hook = ForumFunction::get_hook('ex_add_new_rss_item_info')) ? eval($hook) : null;
...
function output_atom($feed)
{
	...
	echo "\t".'<title type="html"><![CDATA['.ForumFunction::escape_cdata($feed['title']).']]></title>'."\n";
	echo "\t".'<link rel="self" href="'.ForumFunction::forum_htmlencode(ForumFunction::get_current_url()).'" />'."\n";
	...
	($hook = ForumFunction::get_hook('ex_add_new_atom_info')) ? eval($hook) : null;
	...
	{
		echo "\t\t".'<entry>'."\n";
		echo "\t\t\t".'<title type="html"><![CDATA['.ForumFunction::escape_cdata($item['title']).']]></title>'."\n";
		echo "\t\t\t".'<link rel="alternate" href="'.$item['link'].'" />'."\n";
		echo "\t\t\t".'<'.$content_tag.' type="html"><![CDATA['.ForumFunction::escape_cdata($item['description']).']]></'.$content_tag.'>'."\n";
		echo "\t\t\t".'<author>'."\n";
		echo "\t\t\t\t".'<name><![CDATA['.ForumFunction::escape_cdata($item['author']['name']).']]></name>'."\n";

		if (isset($item['author']['email']))
			echo "\t\t\t\t".'<email><![CDATA['.ForumFunction::escape_cdata($item['author']['email']).']]></email>'."\n";
		...
		($hook = ForumFunction::get_hook('ex_add_new_atom_item_info')) ? eval($hook) : null;
...
function output_xml($feed)
{
	...
	($hook = ForumFunction::get_hook('ex_add_new_xml_info')) ? eval($hook) : null;
	...
	foreach ($feed['items'] as $item)
	{
		...
		echo "\t\t".'<title><![CDATA['.ForumFunction::escape_cdata($item['title']).']]></title>'."\n";
		echo "\t\t".'<link>'.$item['link'].'</link>'."\n";
		echo "\t\t".'<content><![CDATA['.ForumFunction::escape_cdata($item['description']).']]></content>'."\n";
		echo "\t\t".'<author>'."\n";
		echo "\t\t\t".'<name><![CDATA['.ForumFunction::escape_cdata($item['author']['name']).']]></name>'."\n";
		...
		($hook = ForumFunction::get_hook('ex_add_new_xml_item_info')) ? eval($hook) : null;
...
function output_html($feed)
{
	...
	foreach ($feed['items'] as $item)
	{
		if (utf8_strlen($item['title']) > FORUM_EXTERN_MAX_SUBJECT_LENGTH)
			$subject_truncated = ForumFunction::forum_htmlencode(ForumFunction::forum_trim(utf8_substr($item['title'], 0, (FORUM_EXTERN_MAX_SUBJECT_LENGTH - 5)))).'…';
		else
			$subject_truncated = ForumFunction::forum_htmlencode($item['title']);

		echo '<li><a href="'.$item['link'].'" title="'.ForumFunction::forum_htmlencode($item['title']).'">'.$subject_truncated.'</a></li>'."\n";
	}
}
...
if ($action == 'feed')
{
	...
	($hook = ForumFunction::get_hook('ex_set_syndication_type')) ? eval($hook) : null;
	...
	if (isset($_GET['tid']))
	{
		...
		($hook = ForumFunction::get_hook('ex_qr_get_topic_data')) ? eval($hook) : null;
		...
		$feed = array(
			...
			'link'			=>	ForumFunction::forum_link($forum_url['topic'], array($tid, ForumFunction::sef_friendly($cur_topic['subject']))),
		...
		($hook = ForumFunction::get_hook('ex_qr_get_posts')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);

		while ($cur_post = $forum_db->fetch_assoc($result))
		{
			if ($forum_config['o_censoring'] == '1')
				$cur_post['message'] = ForumFunction::censor_words($cur_post['message']);
			...
			$item = array(
				...
				'link'			=>	ForumFunction::forum_link($forum_url['post'], $cur_post['id']),
			...
			if ($cur_post['poster_id'] > 1)
			{
				...
				$item['author']['uri'] = ForumFunction::forum_link($forum_url['user'], $cur_post['poster_id']);
			}
			...
			($hook = ForumFunction::get_hook('ex_modify_cur_post_item')) ? eval($hook) : null;
		}

		($hook = ForumFunction::get_hook('ex_pre_topic_output')) ? eval($hook) : null;
	...
	else
	{
		...
		{
			$fids = explode(',', ForumFunction::forum_trim($_GET['fid']));
		...
		{
			$nfids = explode(',', ForumFunction::forum_trim($_GET['nfid']));
		...
		$feed = array(
			...
			'link'			=>	ForumFunction::forum_link($forum_url['index']),
		...
		($hook = ForumFunction::get_hook('ex_qr_get_topics')) ? eval($hook) : null;
		$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
		while ($cur_topic = $forum_db->fetch_assoc($result))
		{
			...
			{
				$cur_topic['subject'] = ForumFunction::censor_words($cur_topic['subject']);
				$cur_topic['message'] = ForumFunction::censor_words($cur_topic['message']);
			}
			...
			$item = array(
				...
				'link'			=>	ForumFunction::forum_link($forum_url['topic_new_posts'], array($cur_topic['id'], ForumFunction::sef_friendly($cur_topic['subject']))),
			...
			if ($cur_topic['poster_id'] > 1)
			{
				...
				$item['author']['uri'] = ForumFunction::forum_link($forum_url['user'], $cur_topic['poster_id']);
			}
			...
			($hook = ForumFunction::get_hook('ex_modify_cur_topic_item')) ? eval($hook) : null;
		}

		($hook = ForumFunction::get_hook('ex_pre_forum_output')) ? eval($hook) : null;
...
else if ($action == 'online' || $action == 'online_full')
{
	...
	($hook = ForumFunction::get_hook('ex_qr_get_users_online')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	while ($forum_user_online = $forum_db->fetch_assoc($result))
	{
		if ($forum_user_online['user_id'] > 1)
		{
			$users[] = $forum_user['g_view_users'] == '1' ?'<a href="'.ForumFunction::forum_link($forum_url['user'], $forum_user_online['user_id']).'">'.ForumFunction::forum_htmlencode($forum_user_online['ident']).'</a>' : ForumFunction::forum_htmlencode($forum_user_online['ident']);
			++$num_users;
		}
		else
			++$num_guests;
	...
	($hook = ForumFunction::get_hook('ex_pre_online_output')) ? eval($hook) : null;
	// Send the Content-type header in case the web server is setup to send something else
	header('Content-type: text/html; charset=utf-8');
	header('Expires: '.gmdate('D, d M Y H:i:s').' GMT');
	header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
	...
	echo $lang_index['Guests online'].': '.ForumFunction::forum_number_format($num_guests).'<br />'."\n";

	if ($_GET['action'] == 'online_full' && !empty($users))
		echo $lang_index['Users online'].': '.implode($lang_index['Online list separator'], $users).'<br />'."\n";
	else
		echo $lang_index['Users online'].': '.ForumFunction::forum_number_format($num_users).'<br />'."\n";
...
else if ($action == 'stats')
{
	...
	($hook = ForumFunction::get_hook('ex_qr_get_user_count')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	($hook = ForumFunction::get_hook('ex_qr_get_newest_user')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	($hook = ForumFunction::get_hook('ex_qr_get_post_stats')) ? eval($hook) : null;
	$result = $forum_db->query_build($query) or ForumFunction::error(__FILE__, __LINE__);
	...
	($hook = ForumFunction::get_hook('ex_pre_stats_output')) ? eval($hook) : null;

	echo sprintf($lang_index['No of users'], ForumFunction::forum_number_format($stats['total_users'])).'<br />'."\n";
	echo sprintf($lang_index['Newest user'], '<a href="'.ForumFunction::forum_link($forum_url['user'], $stats['last_user']['id']).'">'.ForumFunction::forum_htmlencode($stats['last_user']['username']).'</a>').'<br />'."\n";
	echo sprintf($lang_index['No of topics'], ForumFunction::forum_number_format($stats['total_topics'])).'<br />'."\n";
	echo sprintf($lang_index['No of posts'], ForumFunction::forum_number_format($stats['total_posts'])).'<br />'."\n";

	exit;
}


($hook = ForumFunction::get_hook('ex_new_action')) ? eval($hook) : null;
